...
summary: How to Configure NVMe-oF Host for Rocky Linux 10.0 with ONTAP storage
---
= Configure Rocky Linux 10.0 for NVMe-oF with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
This document describes how to configure NVMe over Fabrics (NVMe-oF) hosts for Rocky Linux 10.0. For more support and feature information, see link:hu-nvme-index.html[NVME-oF Overview^]. NVMe-oF with Rocky Linux 10.0 has the following known limitations:
...
. Install Rocky Linux 10.0 on the server. After the installation is complete, verify that you are running the required Rocky Linux 10.0 kernel: 
+
[source,cli]
----
uname -r
...
With Rocky Linux 10.0, the `nvmefc-boot-connections.service` and `nvmf-autoconnect.service` boot services included in the NVMe/FC `nvme-cli` package are automatically enabled when the system boots. 
...
Beginning with ONTAP 9.12.1, secure in-band authentication is supported over NVMe/TCP between a Rocky Linux 10.0 host and an ONTAP controller.
...
cat /etc/nvme/hostnqn
----
. Generate the dhchap key for the Rocky Linux 10.0 host.
+
