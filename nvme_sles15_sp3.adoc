...
----
+
.	Verify that the NVMe/FC initiator ports are enabled, you are able to see the target ports, and all are up and running. In the following example output, only one initiator port is enabled and connected with two target LIFs:
+
[subs=+quotes]
----
