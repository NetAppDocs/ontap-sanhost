...
The maximum size for an I/O request that ONTAP can receive from the host is 1MB. However, if you would like for host to issue an I/O request of that size, you need to increase the the `lpfc` parameter from the default value of `64` to `256`.
...
. Verify that the discovery log page data is available and can be fetched through the initiator port - target LIF combo:
+
----
nvme discover -t tcp -w host-traddr -a traddr
----
...
+
[NOTE]
The above command shows the dhchap host key while the second shows the dhchap ctrl key applied for that specific subsystem on the host.
+
[NOTE]
`nvme list` command returns all available namespaces on the host.
`nvme list-subsys` command returns all available NVMe controllers on the host.
`nvme list-subsys /dev/nvmeXnY` returns all the available subsystems along with its ANA/path states for that particular namespace nvmeXnY on the host.
--
...
Before troubleshooting any NVMe/FC failures, verify that you are running a configuration that is compliant to the Interoperability Matrix Tool (IMT) specifications and then proceed with the next steps to debug any host side issues.
