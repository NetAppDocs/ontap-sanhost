...
= NVMe/FC Host Configuration for Oracle Linux 8.7 with ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:nofooter:
...
*	Oracle Linux 8.7 has in-kernel NVMe multipath enabled for NVMe namespaces by default, therefore, there is no need for explicit settings.

* Oracle Linux 8.7 includes support for NVMe/TCP in addition to NVMe/FC. The NetApp plugin in the native `nvme-cli` package can display ONTAP details for both NVMe/FC and NVMe/TCP namespaces.

* Oracle Linux 8.7 supports use of NVMe and SCSI co-existent traffic on the same host on a given HBA adapter, without the explicit dm-multipath settings to prevent claiming NVMe namespaces.
...
. Verify that you are running the supported adapter driver and firmware versions. The native inbox qla2xxx driver included in the OL 8.7 GA kernel has the latest upstream fixes essential for ONTAP support:
+
----
...
NVMe/FC Host Configuration for Oracle Linux 8.7 with ONTAP release has the following known issues:
