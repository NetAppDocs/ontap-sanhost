...
= Use Oracle Linux 8.7 with ONTAP
:hardbreaks:
:toclevels: 1P
:nofooter:
:icons: font
...
[.lead]
You can use the ONTAP SAN host configuration settings to configure Oracle Linux 8.7 with ONTAP as the target.
...
The following sections provide sample multipath outputs for a LUN mapped to All SAN Array (ASA) and non-ASA configurations.
...
[NOTE] 
To configure Oracle Linux 8.7 Red Hat Compatible Kernel (RHCK), use the link:hu_rhel_87.html#recommended-settings[recommended settings] for Red Hat Enterprise Linux (RHEL) 8.7.
