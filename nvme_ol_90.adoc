...
= NVMe/FC Host Configuration for Oracle Linux 9.0 with ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:nofooter:
...
[.lead]
NVMe over Fabrics (NVMe-oF), including NVMe/FC and other transports, is supported with Oracle Linux 9.0 with Asymmetric Namespace Access (ANA). In NVMe-oF environments, ANA is the equivalent of ALUA multipathing in iSCSI and FC environments and is implemented with in-kernel NVMe multipath. 
...
== Enable NVMe/FC with Oracle Linux 9.0

You can enable the NVMe/FC in-kernel multipath using the following procedure.

.Steps

. Install Oracle Linux 9.0 GA on the server. After the installation is complete, verify that you are running the specified Oracle Linux 9.0 GA kernel. See the link:https://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix^] for the most current list of supported versions.
+
----
# uname -r
----
...
.	On the Oracle Linux 9.0 host, check the `hostnqn` string at `/etc/nvme/hostnqn`: 
+
----
...
----------- --------------- ----------------------------------------------------------
vs_ol_nvme  nvme_ss_ol_1    nqn.2014-08.org.nvmexpress:uuid:edd38060-00f7-47aa-a9dc-4d8ae0cd969a
----
+
[NOTE]
...
. Verify that you are running the supported adapter driver and firmware versions. The native inbox qla2xxx driver included in the OL 8.6 GA kernel has the latest upstream fixes essential for ONTAP support:
+
----
