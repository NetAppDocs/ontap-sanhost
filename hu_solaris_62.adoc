...
[.lead]
The Solaris Host Utilities assist you in managing ONTAP storage attached to a Solaris host. The Solaris Host Utilities supports several Solaris environments and multiple transport protocols.

These are the primary Solaris Host Utilities environments: 
...
The Solaris Host Utilities 6.2 support the following Solaris versions:
...
. Confirm that the toolkit was successfully installed by using the `pkginfo` or `ls - al` command.
+
.Show example output
[%collapsible]
====
