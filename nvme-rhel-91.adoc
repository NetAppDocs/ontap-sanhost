...
This document describes how to configure NVMe over Fabrics (NVMe-oF) hosts for RHEL 9.1. For additional support and feature information, refer to link:hu-nvme-index.html[NVME-oF Overview^]. The following are known limitations for RHEL 9.1:
...
You can use the following procedure to verify the minimum supported RHEL 9.1 software versions and NVMe configuration.
...
[NOTE]
NVMe/FC is supported on RHEL 9.1 and later. NVMe/TCP is supported on RHEL 8.6 and later. NVMe/TCP doesn't support auto-connect operations and requires manual connect or connect-all commands.
...
With RHEL 9.1, the `nvmefc-boot-connections.service` and `nvmf-autoconnect.service` boot services included in the NVMe/FC `nvme-cli` package are automatically enabled when the system boots. 
