...
+
[NOTE]
NetApp recommends setting the `ctrl-loss-tmo` option with an indefinite value (such as -1) so that an NVMe subsystem continues to try reconnecting in the event of a path loss. 
...
DHHC-1:03:J2UJQfj9f0pLnpF/ASDJRTyILKJRr5CougGpGdQSysPrLu6RW1fGl5VSjbeDF1n1DEh 3nVBe19nQ/LxreSBeH/bx/pU=:
----
...
|1490498	| Host is non-operational at `crypto_ahash_digest()` during NVMe reauthentication |During NVMe in-band re-authentication on SUSE Linux Enterprise Server (SLES) 15 SP4 hosts, a host crash frequently occurs at the crypto_ahash_digest() REST API.
|1496739	| Host cannot disconnect after sending `auth_failure2` during re-authentication |None	
|===
