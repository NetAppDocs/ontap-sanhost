...
summary: Use Linux Host Utilities with Veritas Infoscale 8 for RHEL and Oracle Linux hosts to help you manage FC, FCoE, and iSCSI protocol operations with ONTAP LUNs.
---
= Configure Veritas Infoscale 8 for FC, FCoE, and iSCSI with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
You can use Veritas Dynamic Multipathing (VxDMP) to manage ONTAP LUNs.

To ensure that multipathing is configured correctly for your host, you need to claim NetApp LUNs as Veritas multipath devices and verify the Array Support Library (ASL) and the Array Policy Module (APM) configuration.
...
.Before you begin
Ensure your configuration meets the system requirements. See the Interoperability Matrix Tool and the Veritas HCL Matrix.
...
. When you install the Veritas software, it loads the default ASL and APM packages. NetApp recommends that you use the latest supported packages listed on the Veritas support portal
+
...
. Set the protocol timeout values:
+
[role="tabbed-block"]
====
.FC/FCoE
--
Use the default timeout values.
--
.iSCSI 
--
...
. Set the RHEL host "rport" values:
+
Configure `udev rport` values to support the Veritas Infoscale environment in storage failover scenarios. 
+
Create the file `/etc/udev/rules.d/40-rport.rules` with the following file content:
+
----
...
+
In AFF, FAS, or ASA configurations, a single ONTAP LUN shouldn't require more than four paths. Having more than four paths can cause problems during a storage failure.
+
