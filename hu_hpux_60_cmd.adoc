...
summary: Describes how to use the HP-UX Host Utilities 6.0 with ONTAP
---
= Use HP-UX Host Utilities commands to verify ONTAP storage configuration
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
[.lead]
Use the HP-UX Host Utilities 6.0 sample command reference for an end-to-end validation of the ONTAP storage configuration using the Host Utilities tool.

== List all host initiators mapped to host

Retrieve a list of all host initiators mapped to a host.
...
Retrieve a list of all LUNs mapped to a host.
...
Retrieve a list of all LUNs mapped to a host from a specific SVM.
...
Retrieve a list of all attributes of a specified LUN mapped to a host.
...
Retrieve a list of ONTAP LUN attributes by a specified host device filename.
...
Retrieve a list of all SVM target LIF WWPNs attached to a host.
