...
* Beginning with ONTAP 9.13.1, NVMe over Fibre Channel (NVMe/FC) support is added for IBM AIX 7.2 TL5 SP6, AIX 7.3 TL1 SP2 and VIOS 3.1.4.2 releases with SAN boot support for both physical and virtual stacks. See the IBM documentation for more information on setting up SAN boot support.
...
* Virtualization support with NetApp (VIOS/PowerVM) is introduced with VIOS 3.1.4.21. This is supported through NPIV (N_PortID Virtualization) storage virtualization mode only using the Power10 IBM server.
...
IBM MPIO (Multi Path I/O), used for NVMe multipathing, is provided by default when you install the OS .  
