...
Multipathing enables you to configure multiple network paths between the host and storage systems. If one path fails, traffic continues on the remaining paths. Oracle Solaris I/O Multipathing (MPxIO) is enabled by default for Solaris 11.4.
...
. The Solaris 11.4 Host Utilities load the NetApp recommended parameter settings for SPARC and x86_64 processors. For additional Solaris 11.4 system settings, see Oracle DOC ID: 2595926.1.
+
...
====
+
You should also load the following recommended settings if they apply to your storage configuration.
+
[role="tabbed-block"]
====
.MetroCluster
...
. To support 4KB aligned I/O with zpools using ONTAP LUNs, verify the following:

.. Solaris 11.4 is installed with the latest Support Repository Update (SRU).

.. The NetApp logical unit has `lun/host-type` as `Solaris`, independent of the LUN size. 
...
+
The path priorities are displayed against the Access State section for each LUN in the output.
+
...
<Dinil to provide example output>
--
