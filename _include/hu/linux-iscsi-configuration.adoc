To ensure that multipathing is configured correctly for your host, set the `timeout` parameter on your host and confirm that the iSCSI service is running and connecting to the iSCSI targets.
...
. Verify the iSCSI initiator node name located in the `/etc/iscsi/initiatorname.iscsi` file:
+
[source,cli]
----
