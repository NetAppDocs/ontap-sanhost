...
= Using VMware vSphere 6.5 and 6.7 with NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:nofooter:
...
== Introduction
This document provides guidance on ONTAP SAN host configuration settings for the vSphere 6.5.x and 6.7.x releases with FC, FCoE and iSCSI protocols.
...
-------------  ------  --------  ------------  ------  ---------  --------------------------  ----------  -------------  -----------  -----------  ---------------------
VMW_SATP_ALUA          LSI       INF-01-00                        reset_on_attempted_reserve  system      tpgs_on        VMW_PSP_MRU               NetApp E-Series
                                                                                                                                                   arrays with ALUA
                                                                                                                                                   support
VMW_SATP_ALUA          NETAPP                                     reset_on_attempted_reserve  system      tpgs_on        VMW_PSP_RR                NetApp arrays with
                                                                                                                                                   ALUA support
...
[width="99%",cols="21%,20%,14%,27%,18%",options="header",]
|===
|*OS version* |*Issue* |*Internal BURT* |*Support Request* |*Note ID from IMT*
|ESXi 6.5 and ESXi 6.7 and updates |WFC RDM luns fails during testing |1413424  |17572733809 |8727
|ESXi 6.5 and updates and esxi 6.7 and updates |PLOGI issue seen during testing on Emulex adapters |1256473 |19012493807(NetApp has closed the SR need to check why ?) |9520
| | | | |
|===
