...
NOTE: AIX requires that one and only one path be visible to a SAN boot disk until MPIO is enabled. Installing or migrating an AIX boot disk generally requires temporarily restricting the FC zoning for the host to iniclude a single FC initiator and a single FC target. Once the Host Utilities, including the PCM package, are installed the OS can be rebooted to fully enable MPIO. At that point, add the additional FC initiators and FC targets to the zone. You should then run `cfgmgr -v` and the `bosboot -a` commands to update the boot configuration to include all available paths.
