...
+
The management software enables the `sanlun` commands to gather information about the FC HBAs, such as their WWPNs. 

* Verify that the following packages are correctly installed. These packages are required to support the `sanlun fcp show adapter` command:
+
...
. The NetApp Linux Host Utilities software package is available on the NetApp Support Site in a 64-bit .rpm file. Download the 64-bit file from the link:https://mysupport.netapp.com/site/products/all/details/hostutilities/downloads-tab/download/61343/7.1/downloads[NetApp Support Site^] to your host.
