...
== Recommendations for AIX hosts configured with SMBC LUNs

Beginning in ONTAP 9.11.1, AIX hosts configured with SnapMirror Business Continuity (SMBC) LUNs only discover paths from the primary cluster after the LUN map. By default, LUN paths are in the “hidden” state inside SCSI targets even though LUNs from the secondary cluster are mapped to an AIX igroup. To discover the new primary paths after SMBC events such as a planned failover event or an automatic unplanned failover (AUFO) event, you must perform a manual AIX host rescan and follow the host remediation steps in this procedure.

In the following example, only paths from the primary cluster are discovered because the secondary cluster LUN paths are hidden. The example shows the LUN path status when the AIX host is in a steady state.
...
=== AIX host remediation after a planned failover or AUFO event

.About this task

The AIX storage stack does not perform an automatic rescan of the LUNS.  After a planned failover or AUFO event, the following AIX host remediation steps must be performed to discover the LUN paths from the new primary cluster.

NOTE: If you do not perform the AIX host remediation steps after a failover or AUFO event, path loss or I/O errors can occur.
...
`#cfgmgr`
+
You can also perform a rescan for a specific host bus adapter (HBA):
`cfgmgr -l fcsx`
...
. Remove the failed paths from the remote cluster for all devices:
+
