...
This document describes how to configure NVMe over Fabrics (NVMe-oF) hosts for RHEL 9.4. For additional support and feature information, refer to link:hu-nvme-index.html[NVME-oF Overview^]. There are no known limitations for RHEL 9.4.
...
Use the following procedure to validate that your RHEL 9.4 system meets the software requirements and verify the NVMe package installations and host configuration.

.Steps

. Install RHEL 9.4 on the server. After the installation is complete, verify that you are running the required RHEL 9.4 kernel: 
+
[source,cli]
----
uname -r
...
[NOTE]
NVMe/TCP doesn't support auto-connect operations and requires manual connect or connect-all commands.
...
With RHEL 9.4, the `nvmefc-boot-connections.service` and `nvmf-autoconnect.service` boot services included in the NVMe/FC `nvme-cli` package are automatically enabled when the system boots. 
...
Beginning with ONTAP 9.12.1, secure in-band authentication is supported over NVMe/TCP between a RHEL 9.4 host and an ONTAP controller.
...
cat /etc/nvme/hostnqn
----
. Generate the dhchap key for the RHEL 9.4 host.
+
