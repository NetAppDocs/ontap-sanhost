...
Beginning with RHEL 10.1, the default iopolicy is queue-depth. In RHEL 10.0, round-robin is the default iopolicy, but queue-depth is available as a configurable option.

If you are using RHEL 10.0 and want to change the iopolicy to queue-depth, modify the udev rules file using this procedure.
