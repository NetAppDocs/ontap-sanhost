...
---
= Configure Rocky Linux 9.5 for NVMe-oF with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
Learn how to configure NVMe over Fabrics (NVMe-oF) hosts for Rocky Linux 9.5. For more support and feature information, see link:nvme-rockylinux-supported-features.html[Rocky Linux ONTAP support and features].

NVMe-oF with Rocky Linux 9.5 has no known limitations.
...
. Install Rocky Linux 9.5 on the server. After the installation is complete, verify that you are running the required Rocky Linux 9.5 kernel: 
+
[source,cli]
----
uname -r
