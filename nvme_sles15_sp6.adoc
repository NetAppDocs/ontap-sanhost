...
[.lead]
NetApp SAN host configurations support the NVMe over Fabrics (NVMe-oF) protocol on SUSE Linux Enterprise Server 15 SP6 with Asymmetric Namespace Access (ANA). You can use NVMe-oF with ONTAP LUNs. In NVMe-oF environments, ANA is equivalent to ALUA multipathing in iSCSI and FCP environments; it is implemented with the in-kernel NVMe multipath feature.
...
----
====
. Generate the TLS PSK by using `nvme gen-tls-key`:

.. Generate `nvme gen-tls-key`:  
+
`# cat /etc/nvme/hostnqn`
+
----
