...
You must install the AIX Host Utilities Kit while using AIX MPIO with ONTAP storage.
...
The AIX Host Utilities 6.1 release addresses the memory leak issue that was reported in the previous releases. Refer to the release notes section for additional information.
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1307653[1307653^] | Seeing I/O issues on VIOS 3.1.1.10 during SFO faults and straight I/O | On VIOS 3.1.1 IO failures might be seen on NPIV client disk which are backed by 16/32Gb FC adapters.  Also, a `vfchost` driver might get into a state where it stops processing I/O requests from the client. Applying IBM APAR IJ22290 IBM APAR IJ23222 will fix the issue. | NA
