. NetApp also recommends making changes to the default settings for MetroCluster, Oracle Solaris virtualization, or SnapMirror active sync if one or more of these options are included in you storage configuration.
+
[role="tabbed-block"]
=====
.MetroCluster
