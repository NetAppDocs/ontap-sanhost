---
sidebar: sidebar
permalink: nvme-rockylinux-84.html
keywords: nvme, linux, rocky linux, enterprise
summary: How to Configure NVMe-oF Host for Rocky Linux 8.4 with ONTAP
---
= Configure Rocky Linux 8.4 for NVMe-oF with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
:linkattrs:
:imagesdir: ./media/

[.lead]
include::_include/nvme/nvme-rocky-linux-introduction.adoc[]

Learn how to configure NVMe over Fabrics (NVMe-oF) hosts for Rocky Linux 8.4. For more support and feature information, see link:nvme-rockylinux-supported-features.html[Rocky Linux ONTAP support and features].

*NVMe-oF with Rocky Linux 8.4 has the following known limitations:*

* SAN booting using the NVMe-oF protocol is not currently supported.
* In-kernel NVMe multipath is disabled by default on NVMe-oF hosts in Rocky Linux 8.4; you must enable it manually.
* NVMe/TCP is available as a technology preview due to known issues.

== Step 1: Optionally, enable SAN booting

include::_include/nvme/enable-san-booting.adoc[]

== Step 2: Verify the software version and NVMe configuration

include::_include/nvme/verify-software-version-introduction.adoc[]

.Steps

. Install Rocky Linux 8.4 on the server. After the installation is complete, verify that you are running the required Rocky Linux 8.4 kernel: 
+
[source,cli]
----
uname -r
----
+
Example Rocky Linux kernel version:
+
----
5.14.0-570.12.1.el9_6.x86_64
----

include::_include/nvme/linux-validate-software-version-rockylinux.adoc[]

== Step 3: Configure NVMe/FC and NVMe/TCP

include::_include/nvme/nvme-configure.adoc[]

== Step 4: Optionally, enable 1MB I/O for NVMe/FC

include::_include/nvme/nvme-enable-1mb-io.adoc[]

== Step 5: Verify the multipathing configuration

include::_include/nvme/nvme-validate-nvme-of-rocky-linux.adoc[]

== Step 6: Review the known issues

There are no known issues.

// JIRA-1289 20-Sep-2023
