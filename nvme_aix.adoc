...
[.lead]
You can enable NVMe/FC on IBM AIX and VIOS/PowerVM hosts using ONTAP storage as the target. For additional details on supported configurations, see the NetApp Interoperability Matrix Tool.
...
* NVMe/FC is supported only with Power 9 and Power 10 IBM servers.
...
* SAN booting is supported with both Physical and Virtual stack. Follow IBM documentation to know the prerequisites and set up SAN boot.
