...
[.lead]
Windows Host Utilities require certain registry and parameter settings to verify that the Windows host correctly handles the storage system behavior.

Windows Host Utilities set parameter values that affect how the Windows host responds to a delay or loss of data. These values have been selected to verify that the Windows host correctly handles events such as the failover of one controller in the storage system to its partner controller.

Some values do not apply to the device-specific module (DSM) for SANtricity Storage Manager. Overlap between values set by Windows Host Utilities and the DSM for SANtricity Storage Manager don't result in conflicts.
...
The Windows Host Utilities installer automatically sets registry values and NVMe parameters that are based on the choices that you make during the installation. You need to review these values and the operating system version. 
...
The Windows Host Utilities installer automatically sets registry values that are based on the choices that you make during installation. You need to review these registry values and the operating system version.
