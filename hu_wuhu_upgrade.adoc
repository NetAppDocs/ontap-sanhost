...
[.lead]
You can upgrade Windows Host Utilities by following the installation instructions interactively or by using the Windows command line. You need to install the new Windows Host Utilities installation software on a path that is accessible by the Windows host. 
