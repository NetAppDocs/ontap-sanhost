To ensure that multipathing is configured correctly for your host, verify that the `/etc/multipath.conf` file is exits and that you have the NetApp recommended settings configured for your ONTAP LUNs.
...
. The settings for Oracle Linux UEK with and without ALUA updates automatically. 
+
Enable "ALUA Handler":
+
