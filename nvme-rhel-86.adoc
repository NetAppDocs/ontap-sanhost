...
---
= Configure RHEL 8.6 for NVMe-oF with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
This document describes how to configure NVMe over Fabrics (NVMe-oF) hosts for RHEL 8.6. For additional support and feature information, refer to link:hu-nvme-index.html[NVME-oF Overview^]. The following are known limitations for RHEL 8.6:

* SAN booting using the NVMe-oF protocol is not currently supported.
* In-kernel NVMe multipath is disabled by default on NVMe-oF hosts in RHEL 8.6; you must enable it manually.
...
You can use the following procedure to validate the minimum supported RHEL 8.6 software versions.

.Steps

. Install RHEL 8.6 on the server. After the installation is complete, verify that you are running the required RHEL 8.6 kernel: 
+
[source,cli]
----
uname -r
----
+
The following example shows a RHEL 8.6 kernel version:
+
----
...
. On the RHEL 8.6 host, check the `hostnqn` string at `/etc/nvme/hostnqn`:
+
[source,cli]
----
...
====
+
NOTE: If the `hostnqn` strings do not match, you should use the `vserver modify` command to update the `hostnqn` string on your corresponding ONTAP storage system NVMe subsystem to match the `hostnqn` string `/etc/nvme/hostnqn` on the host.
