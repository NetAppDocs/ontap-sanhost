...
This document describes how to configure NVMe over Fabrics (NVMe-oF) hosts for RHEL 9.2. For additional support and feature information, refer to link:hu-nvme-index.html[NVME-oF Overview^]. The following are known limitations for RHEL 9.2:
...
Use the following procedure to validate that your RHEL 9.2 system meets the software requirements and verify the NVMe package installations and host configuration.

.Steps

. Install RHEL 9.2 on the server. After the installation is complete, verify that you are running the required RHEL 9.2 kernel: 
+
[source,cli]
----
uname -r
...
With RHEL 9.2, the `nvmefc-boot-connections.service` and `nvmf-autoconnect.service` boot services included in the NVMe/FC `nvme-cli` package are automatically enabled when the system boots. 
