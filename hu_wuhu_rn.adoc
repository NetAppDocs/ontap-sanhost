...
|To verify that all of your iSCSI paths are operational after a reboot, use either all IPv4 endpoints or all IPv6. Do not mix IPv4 and IPv6.
|MCS is not supported with iSCSI software boot |When using the iSCSI software boot with iSCSI multiple connections per session (MCS), the iSCSI boot leading connection cannot be protected. This can lead to disconnection from the boot LUN causing host system disruption.
