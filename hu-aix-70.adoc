...
+
`bash-4.1$ tar -xvf ntap_aix_host_utilities_7.0.tar.gz`
...
+
`bash-4.1$ installp -aXYd /var/tmp/ntap_aix_host_utilities_7.0/MPIO NetApp.MPIO_Host_Utilities_Kit`

. Install the SAN toolkit:
+
`bash-4.1$ installp -aXYd /var/tmp/ntap_aix_host_utilities_7.0/SAN_Tool_Kit/NetApp.SAN_toolkit`

. Reboot the host.

. Verify the installation:
+
`bash-4.1$ lslpp -l |grep -i netapp`
+
----
...
+
`bash-4.1$ odmget -q "uniquetype=disk/fcp/NetAppMPIO" PdAt |grep  "lbp_enabled"`
+
----
attribute = "lbp_enabled"
----
+
`bash-4.1$ odmget -q "uniquetype=disk/fcp/NetAppMPIO and attribute=lbp_enabled" PdAt`
+
----
PdAt:
