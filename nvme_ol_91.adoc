...
NVMe-oF Host Configuration for Oracle Linux 9.1 with ONTAP has the following known issues:
...
|1539101 |NVMe Discover -p command not creating persistent discovery subsystem | On Oracle Linux 9.1 NVMe over Fabrics (NVMe-oF) hosts, you can use the nvme discover -p command to create Persistent Discovery Controllers (PDCs). When this command is used, one PDC should be created per initiator-target combination. However, if you are running ONTAP 9.12.1 and Oracle Linux 9.1 with an NVMe-oF host, PDC creation fails when the `nvme discover -p` command is executed. 
