...
* By default, the Asymmetric Namespace Access Transition Timeout (ANATT) value for the AIX host OS is 30 seconds. IBM provides an Interim Fix (ifix) which caps the ANATT value at 60 seconds; you need to install an ifix from the IBM website to ensure that all ONTAP workflows are non-disruptive. 
+
...
+
The following table shows HBAs assigned to the AIX LPAR (AIX Logical Partition) or the physical stack:
+
...
|===
+
The following table shows HBAs assigned to the VIOS with NPIV-enabled support in a virtualized mode:
+
[cols="10,10,10,10,10",options="header"]
|===
|Host OS	|Power Arch	|Power FW version	|Mode	|Comments
...
Before troubleshooting any NVMe/FC failures, verify that you are running a configuration that is compliant with the Interoperability Matrix Tool (IMT) specifications. If you are still facing issues, contact link:https://mysupport.netapp.com[NetApp support^].