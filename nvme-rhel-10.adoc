...
** Beginning with ONTAP 9.12.1, support for secure in-band authentication is introduced for NVMe/TCP. You can use secure in-band authentication for NVMe/TCP with RHEL 10.0.
...
* Features available:
** Beginning with RHEL 10.0, native NVMe multipathing is always enabled, and DM multipath support for NVMe-oF is not supported.
* Known limitations: 
