...
.What you'll need
If you decide to use SAN booting, it must be supported by your configuration. You can use the link:https://mysupport.netapp.com/matrix/imt.jsp?components=83635;&solution=1&isHWU&src=IMT[NetApp Interoperability Matrix Tool^] to verify that your OS, HBA, HBA firmware and the HBA boot BIOS, and ONTAP version are supported.
...
NOTE: To configure Oracle Linux 7.5 RedHat Enterprise Kernel (RHCK), use the link:hu_rhel_75.html#recommended-settings[recommended settings] for Red Hat Enterprise Linux (RHEL) 7.5.
...
NOTE: For Oracle Linux (Red Hat compatible kernel) known issues, see the link:hu_rhel_75.html#known-problems-and-limitations[known issues] for Red Hat Enterprise Linux (RHEL) 7.5.
