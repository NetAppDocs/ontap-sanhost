...
If your host is connected to LUNs from other vendors and any of the multipath parameter settings are overridden, you need to correct them by adding stanzas in the `multipath.conf` file that apply specifically to ONTAP LUNs. You need to scroll down in the file to add these stanzas. If you don't correct the parameters, the ONTAP LUNs might not work as expected.
