...
Beginning with Proxmox VE 9.0, the default iopolicy for NVMe-oF is set to `queue-depth`. If you want to change the iopolicy to `round-robin`, which is also available as a configurable option, modify the udev rules file as follows:
...
----
. Modify the rule so that `queue-depth` is changed to `round-robin`, as shown in the following example:
+
----
...
. Verify the current iopolicy for your subsystem. Replace <subsystem> with the name of your subsystem, for example, nvme-subsys0.
+
[source,cli]
----
...
. Run the `update-initramfs` command, and reboot the host.
...
.Show AFF, FAS, or ASA example output
[%collapsible]
====
[subs=+quotes]
----
