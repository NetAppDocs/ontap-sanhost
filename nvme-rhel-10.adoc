...
This document describes how to configure NVMe-oF hosts for Red Hat Enterprise Linux (RHEL) 10.0. For additional support and feature information, refer to link:hu_nvme_index.adoc[NVME-oF Overview^]. The following are known limitations for RHEL 10.0:
...
You can configure your host to use SAN booting to simplify deployment and improve scalability. Refer to link:nvme-enable-SAN-booting.adoc[Enable SAN booting^].

== Step 2: Validate software versions

Use the following procedure to validate the minimum supported RHEL 10.0 software versions.

.Steps

. Install RHEL 10.0 on the server. After the installation is complete, verify that you are running the specified RHEL 10.0 kernel: 
+
[source,cli]
----
uname -r
...
You can increase the lpfc value to issue 1MB I/O requests on the Broadcome NVMe/FC host. Refer to  link:nvme-enable-broadcom-1mb-size.adoc[Enable 1MB I/O^].
