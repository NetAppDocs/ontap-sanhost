...
[.lead]
You can configure Rocky Linux 8.4 with NVMe protocols to manage ONTAP LUNs.
[.lead]
...
* Features available:
** Beginning with ONTAP 9.12.1, you can use secure in-band authentication for NVMe-oF with Rocky Linux 8.4.
** Rocky Linux 8.4 enables in-kernel NVMe multipath for NVMe namespaces by default, removing the need for explicit settings.
* Known limitations: 
