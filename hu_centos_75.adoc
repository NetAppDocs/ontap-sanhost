...
summary: Describes how to use CentOS 7.5 with ONTAP
---

= Use CentOS 7.5 with ONTAP
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
[.lead]
You can use the ONTAP SAN host configuration settings to configure CentOS 7.5 with ONTAP as the target.
...
.Before you begin
If you decide to use SAN booting, it must be supported by your configuration. You can use the link:https://mysupport.netapp.com/matrix/imt.jsp?components=82985;&solution=1&isHWU&src=IMT[Interoperability Matrix Tool^] to verify that your OS, HBA, HBA firmware and the HBA boot BIOS, and ONTAP version are supported.

include::_include/hu/reuse_hu_san_booting_steps.adoc[]

== Multipathing

For CentOS 7.5 the /etc/multipath.conf file must exist, but you do not need to make specific changes to the file. CentOS 7.5 is compiled with all settings required to recognize and correctly manage ONTAP LUNs.
...
The CentOS 7.5 OS is compiled to recognize ONTAP LUNs and automatically set all configuration parameters correctly for both ASA and non-ASA configuration.
