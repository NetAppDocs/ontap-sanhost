...
For non-ASA configuration, you should see Active/Optimized (primary) paths and Active/non-optimized (secondary) paths.  The Active/Optimized paths are local and serviced by the controller where the aggregate is located. The non-optimized paths are remote and served from a different controller (HA-partner). The non-optimized paths are only used when no Optimized paths are available.
