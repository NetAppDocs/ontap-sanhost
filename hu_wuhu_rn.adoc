...
|Multiple connections per second (MCS) is not supported with iSCSI software boot |When using the iSCSI software boot with iSCSI MCS, the iSCSI boot leading connection cannot be protected. This can lead to disconnection from the boot LUN and cause host system disruption.
