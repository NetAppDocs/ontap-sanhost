...
[.lead]
Red Hat Enterprise Linux (RHEL) 9.5 supports NVMe over Fabrics (NVMe-oF), including NVMe over Fibre Channel (NVMe/FC) and other transports, with Asymmetric Namespace Access (ANA). In NVMe-oF environments, in-kernel NVMe multipath implements ANA, which is the equivalent of asymmetric logical unit access (ALUA) multipathing in iSCSI and FC environments.

== About this task 
ONTAP provides the following support for NVMe-oF host configuration for RHEL 9.5. You should also review the known limitations before starting the configuration process.
...
NVMe/TCP does not have auto-connect functionality. Therefore, you should use the following procedure to discover the NVMe/TCP subsystems and namespaces by performing the NVMe/TCP `connect` or `connect-all` functionality manually.
