...
The The NVMe-oF host configuration for RHEL 8.7 with ONTAP release has the following known issues:
...
Before commencing any troubleshooting for any NVMe/FC failures, make sure you are running a configuration that is compliant to the Interoperability Matrix Tool (IMT) specifications and then proceed with the following steps to debug any host side issues.
