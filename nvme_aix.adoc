...
* No separate PCM (Path Control Module), such as Host Utilities for AIX SCSI Microsoft Multipath I/O (MPIO) support, is required for NVMe devices.
...
+
The following table demonstrates HBAs assigned to the AIX LPAR (AIX Logical Partition) or the physical or virtual stack in a non-virtualized mode:
+
