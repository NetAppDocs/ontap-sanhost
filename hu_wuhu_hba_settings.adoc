...
[NOTE]
====
You need to change the LimTransferSize parameter value after installing Windows Host Utilities 8.0 or 7.2, see <<nvme_parameters, NVMe parameters>>.

For all other parameters and registry keys for Windows Host Utilities 8.0, 7.2, or 7.1, you don't need to change the values unless technical support tells you to do so.
====

== Registry values set by Windows Host Utilities 8.0 or 7.2
...
|120
|When MPIO support is specified and your server is Windows Server 2025, 2022, 2019, 2016, or 2012 R2 (Windows Host Utilities 7.2 only)
...
|30
|When your server is Windows Server 2025, 2022, 2019, 2016, or 2012 R2  (Windows Host Utilities 7.2 only)
|HKLM\SYSTEM\CurrentControlSet\Services\mpio
...
The following NVMe Emulex driver parameters are updated when installing Windows Host Utilities 8.0 or 7.2:

* EnableNVMe = 1
* NVMEMode = 0
* LimTransferSize=1
+
The LimTransferSize parameter is automatically set to "1" when you install Windows Host Utilities 8.0 or 7.2. After installation, you need to manually change the LimTransferSize value to "0" and reboot the server.
--
====
