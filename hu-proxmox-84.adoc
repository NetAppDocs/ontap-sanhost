---
sidebar: sidebar
permalink: hu-proxmox-84.html
keywords: host utilities, proxmox, linux, 8.4, netapp, ontap
summary: Install the Linux Host Utilities on a Proxmox 8.4 host to manage FCP and iSCSI protocol operations with ONTAP LUNs.
---
= Configure Proxmox 8.4 for FCP and iSCSI with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
[.lead]
The Linux Host Utilities software provides management and diagnostic tools for Linux hosts that are connected to ONTAP storage. When you install the Linux Host Utilities on an Proxmox 8.4 host, you can use the Host Utilities to help you manage FCP and iSCSI protocol operations with ONTAP LUNs.

include::_include/hu/linux-enable-san-boot.adoc[]

== Step 2: Confirm the multipath configuration for your host

You can use multipathing with Proxmox 8.4 to manage ONTAP LUNs.
...
* Learn about configuring the Proxmox storage pools and volumes.
