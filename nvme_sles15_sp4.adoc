...
[.lead]
NVMe over Fabrics (NVMe-oF) is supported with SUSE Linux Enterprise Server (SLES) 15 SP4.
With ONTAP 9.5 and later, Asymmetric Namespace Access (ANA) is enabled by default for multipathing. ANA is the NVMe equivalent of ALUA for FC and iSCSI.
...
. Verify that you have the recommended adapter model:
+
----
...
. Verify that you have the recommended Emulex HBA firmware versions:
+
----
...
. Verify that you have the recommended LPFC driver version:
+
----
...
The maximum size for an I/O request that ONTAP can receive from the host is 1MB. However, if you would like for host to issue an I/O request of that size, you must increase the the lpfc parameter from the default value of 64 to 256.

.Steps

. In the lpfc.conf file, set the lpfc value to 256:
+
----
cat /etc/modprobe.d/lpfc.conf options lpfc lpfc_sg_seg_cnt=256
----
. Run a `dracut -f` command and reboot the host.
. After reboot, verify that the lpfc value is 256:
+
----
...
. Verify that other NVMe/TCP initiator-target LIF combos can successfully fetch discovery log page data. 
+
----
nvme discover -t tcp -w host-traddr  -a traddr
----
...
. Run nvme connect-all command across all the supported NVMe/TCP initiator-target LIFs across the nodes, and set the controller loss timeout period for at least 30 minutes or 1800 seconds:
+
----
...
# nvme connect-all -t tcp -w 192.168.2.5 -a 192.168.1.36 -l 1800
# nvme connect-all -t tcp -w 192.168.2.5 -a 192.168.1.37 -l 1800
----
...
----
+
Example output for NVMe/FC hosts:
+
----
...
Example output for NVMe/TCP hosts:
+
----
...
+- nvme31 tcp traddr=192.168.1.32 trsvcid=4420,host_traddr=192.168.2.5 live non-optimize live non-optimized
----
...
== Discovery Hardening - TP 8013 and TP 8014 (with ONTAP 9.11.1)

Beginning with ONTAP 9.11.1, you create a persistent discovery controller for your SLES 15 SP 4 host.

.Steps

. Verify the following <must fill in the accurate instruction>:
...
Beginning with ONTAP 9.12.1 and later secure, in-band authentication is supported over NVMe/TCP between your SLES 15 SP host and your ONTAP controller.
...
You can setup secure in-band authentication using the CLI or using a config JSON file. 
If you need to specify different dhcap keys for different subsystems, then you must use a config JSON file. 
...
3.	Generate the dhchap key for the controller:
nvme gen-dhchap-key -s optiona_secret -l key_length {32|48|64}  -m HMAC_function {0|1|2|3}  -n host_nqn 

Note: During NVMe connect, you can use the -S option to input your user-defined dhchap key for the host and the -C option to input the dhchap key for the controller.

4.	On the ONTAP controller, add the host and specify both dhchap keys:

vserver nvme subsystem host add -vserver svm_name -subsystem subsystem -host-nqn host_nqn -dhchap-host-secret authentication_host_secret -dhchap-controller-secret authentication_controller_secret -dhchap-hash-function {sha-256|sha-512} -dhchap-group {none|2048-bit|3072-bit|4096-bit|6144-bit|8192-bit}

5.	On the host, connect to the ONTAP controller and specify both dhchap keys:

nvme connect -t tcp -w ip_address -a ip_address -n host_nqn -S authentication_host_secret -C authentication_controller_secret 


6.	Verify that the authentication keys are registered in the host sysfs file:
nvme list
nvme list-subsys
nvme list-subsys /dev/nvem0n1
# cat /sys/class/nvme-subsystem/nvme-subsys0/nvme0/dhchap_secret DHHC-1:01:NunEWY7AZlXqxITGheByarwZdQvU4ebZg9HOjIr6nOHEkxJg:

# cat /sys/class/nvme-subsystem/nvme-subsys0/nvme0/dhchap_ctrl_secret DHHC-
1:03:2YJinsxa2v3+m8qqCiTnmgBZoH6mIT6G/6f0aGO8viVZB4VLNLH4z8CvK7pVYxN6S5f OAtaU3DNi12rieRMfdbg3704=:
...
{
"hostnqn":"nqn.2014-08.org.nvmexpress:uuid:12372496-59c4-4d1b- be09-"hostid":"3ae10b42-21af-48ce-a40b-cfb5bad81839",
...
"subsystems":[
{
"nqn":"nqn.1992- 08.com.netapp:sn.48391d66c0a611ecaaa5d039ea165514:subsystem.subsys_CLIEN T116",
"ports":[
{
"transport":"tcp", "traddr":"192.168.1.117",
"host_traddr":"192.168.1.16", "trsvcid":"4420", "dhchap_ctrl_key":"dhchap_ctrl_secret"
},
{
"transport":"tcp", "traddr":"192.168.1.116",
"host_traddr":"192.168.1.16", "trsvcid":"4420", "dhchap_ctrl_key":" dhchap_ctrl_secret "
},
{
"transport":"tcp", "traddr":"192.168.2.117",
"host_traddr":"192.168.2.16", "trsvcid":"4420", "dhchap_ctrl_key":" dhchap_ctrl_secret "
},
{
"transport":"tcp", "traddr":"192.168.2.116",
"host_traddr":"192.168.2.16", "trsvcid":"4420", "dhchap_ctrl_key":"DHHC-
1:01:0h58bcT/uu0rCpGsDYU6ZHZvRuVqsYKuBRS0Nu0VPx5HEwaZ:"
}
]

2.	Connect to the ONTAP controller using the config JSON file.
The discovery options from the/etc/nvme/discovery.conf file are automatically selected
...
Example output
nvme-subsys1 - NQN=nqn.1992- 08.com.netapp:sn.48391d66c0a611ecaaa5d039ea165514:subsystem.subsys_CLIEN T116
\
...
5.	nvme list-subsys /dev/nvme1n1 
...
If you have an issue with your configuration, verbose logging can provide valuable information essential for troubleshooting.

.Steps

The procedure to set verbose logging for Qlogic (Qla2xxx) is different from the procedure to set LFC verbose logging.
...
|1490498	| Host crash at `crypto_ahash_digest()` during NVMe reauthentication |None
|1496739	| Host fails to disconnect after sending `auth_failure2` during reauthentication |None	
|===
