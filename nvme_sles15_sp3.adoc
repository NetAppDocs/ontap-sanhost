...
[options=""]
|===
| Broadcom/Emulex a| *	32G FC Gandalf LPe32002 adapters used as NVMe/FC initiators on the SLES15 SP3 host
...
| Marvell/QLogic
a| *	32G FC QLE2742 adapters used as NVMe/FC initiators on the SLES15 SP3 NVMe/FC ANA host
...
.	Install the link:http://nbsweb.eng.btc.netapp.in/u/marting/web/SLES15SP3/kernel/5.3.18-59.5.2/[SLES15 SP3 MU kernel-default-5.3.18-59.5.2] separately to avail the latest kernel fixes:
+
----
...
.	Next, upgrade to link:http://nbsweb.eng.btc.netapp.in/u/marting/web/SLES15SP3/nvme-cli/1.13-3.3.1/[nvme-cli-1.13-3.3.1], by running  `rpm -Uvh` command on this package:
+
----
...
----
+
**	The newer lpfc drivers (both inbox and outbox) already have lpfc_enable_fc4_type default set to 3, therefore, you no longer need to set this explicitly in the `/etc/modprobe.d/lpfc.conf` anymore, and recreate the `initrd`. The `lpfc nvme` support is already enabled by default:
+
----
