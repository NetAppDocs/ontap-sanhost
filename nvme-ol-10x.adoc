...
---
= Configure Oracle Linux 10 with NVMe-oF for ONTAP storage 
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
Learn how to configure NVMe over Fabrics (NVMe-oF) hosts for Oracle Linux 10. For more support and feature information, see link:nvme-ol-supported-features.html[Oracle Linux ONTAP support and features].

NVMe-oF with Oracle Linux 10 has the following known limitation:
...
Use the following procedure to validate the minimum supported Oracle Linux 10 software versions.

.Steps

. Install Oracle Linux 10 on the server. After the installation is complete, verify that you are running the specified Oracle Linux 10 kernel. 
+
[source,cli]
----
uname -r
...
.	On the Oracle Linux 10 host, check the `hostnqn` string at `/etc/nvme/hostnqn`: 
+
[source,cli]
----
...
The Oracle Linux 10 host sets the default iopolicy for NVMe-oF to `queue-depth`. Beginning with Oracle Linux 9.6, you can change the iopolicy to `round-robin` by modifying the udev rules file.
...
Secure in-band authentication is supported over NVMe/TCP between a Oracle Linux 10 host and an ONTAP controller.
