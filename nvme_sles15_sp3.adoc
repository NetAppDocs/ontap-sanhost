...
*	There is no sanlun support for NVMe-oF. Therefore, there is no Linux Unified Host Utilities support for NVMe-oF on SUSE Linux Enterprise Server 15 SP3. You can rely on the NetApp plug-in included in the native nvme-cli package for NVMe-oF. This should support all NVMe-oF transports.
