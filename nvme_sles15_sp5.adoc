...
In-kernel NVMe multipath is already enabled by default on SLES hosts such as SLES15 SP4. Therefore, no additional setting is required here. Refer to the NetApp Interoperability Matrix for accurate details regarding supported configurations.

== NVMe-oF initiator packages
Refer to the NetApp Interoperability Matrix for accurate details regarding supported configurations.

1.	Verify that you have the requisite kernel and nvme-cli MU packages installed on the SLES15 SP4 MU host.
+
----
Example:
# uname -r
