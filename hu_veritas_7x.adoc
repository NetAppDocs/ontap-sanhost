...
keywords: host utilities, veritas, infoscale, 7.4.a, 7.4.2, 7.4.3, netapp, ontap
summary: Describes how to use Oracle Linux 6.4 with ONTAP
---

= Using Veritas Infoscale 7.4.x with NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:nofooter:
...
== Introduction
This document provides guidance on ONTAP SAN host configuration settings for Veritas Infoscale Product 7.4.1, 7.4.2, 7.4.3 releases for Red Hat Enterprise Linux & Oracle Linux (RHCK based) platforms with FC, FCoE and iSCSI protocols.
...
Refer to Veritas Support Portal (Product Matrix; Platform Lookup; HCL Matrix) to ensure SAN Boot configuration supportability and known caveats.
...
When you are using Veritas Dynamic Multipathing (VxDMP), you must perform configuration tasks to claim NetApp LUNs as Veritas Multipath Devices. You must have the Array Support Library (ASL) and the Array Policy Module (APM) packages installed, that Veritas provides for NetApp storage systems.  While the Veritas software installation loads the default ASL APM packages, it is recommended to use the latest supported packages listed on Veritas support.
...
This section provides sample multipath output for a LUN mapped to a non-ASA configuration.
...
The following Veritas VxDMP tunables are recommended by NetApp:
...
The following example the effective VxDMP tunables on the SAN host.
...
+
The iSCSI `replacement_timeout` parameter controls how long the iSCSI layer should wait for a timed-out path or session to reestablish itself before failing any commands on it.
...
For Red Hat Enterprise Linux 7 and 8 series, you must configure `udev rport` values to support Veritas Infoscale environment in storage failover scenarios. Create the file /etc/udev/rules.d/40-rport.rules with the following illustrated file content:
...
NOTE: For all other settings specific to Veritas, refer to the standard Veritas Infoscale product documenation.
