...
summary: Describes how to use Red Hat Enterprise Linux 7.8 with ONTAP
---

= Use Red Hat Enterprise Linux 7.8 with ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:nofooter:
...
[.lead]
You can use the ONTAP SAN host configuration settings to configure Red Hat Enterprise Linux 7.8 with ONTAP as the target. 
...
For Red Hat Enterprise Linux (RHEL) 7.8 the /etc/multipath.conf file must exist, but you do not need to make specific changes to the file. RHEL 7.8 is compiled with all settings required to recognize and correctly manage ONTAP LUNs.
...
The RHEL 7.8 OS is compiled to recognize ONTAP LUNs and automatically set all configuration parameters correctly for both ASA and non-ASA configuration.
...
The RHEL 7.8 with ONTAP has the following known issues:
