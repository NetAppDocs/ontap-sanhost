...
[.lead]
You can use the ONTAP SAN host configuration settings to configure Red Hat Enterprise Linux 9.5 with ONTAP as the target. 
...
.Before you begin
Use the link:https://mysupport.netapp.com/matrix/imt.jsp?components=91241;&solution=236&isHWU&src=IMT[NetApp Interoperability Matrix Tool^] to verify that your OS, HBA, HBA firmware, and the HBA boot BIOS, and the ONTAP version are supported.

include::_include/hu/reuse_hu_san_booting_steps.adoc[]

== Multipathing

For Red Hat Enterprise Linux (RHEL) 9.5, the `/etc/multipath.conf` file must exist, but you do not need to make specific changes to the file. RHEL 9.5 is compiled with all the settings that are required to recognize and correctly manage ONTAP LUNs.
