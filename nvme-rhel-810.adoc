...
This document describes how to configure NVMe-oF hosts for Red Hat Enterprise Linux (RHEL) 8.10. For additional support and feature information, refer to link:hu-nvme-index.html[NVME-oF Overview^]. The following are known limitations for RHEL 8.10:
...
You can use the following procedure to validate the minimum supported RHEL 8.10 software versions.

.Steps

. Install RHEL 8.10 on the server. After the installation is complete, verify that you are running the specified RHEL 8.10 kernel: 
+
[source,cli]
----
uname -r
...
+
----
The driver version included in the RHEL 8.10 kernel.
----
