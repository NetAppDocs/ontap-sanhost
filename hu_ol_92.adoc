...
summary: Describes how to use Oracle Linux 9.2 with ONTAP
---

= Use Oracle Linux 9.2 with ONTAP
:hardbreaks:
:toclevels: 1P
:nofooter:
:icons: font
...
[.lead]
You can use the ONTAP SAN host configuration settings to configure Oracle Linux 9.2 with ONTAP as the target.
...
For OL 9.2 the `/etc/multipath.conf` file must exist, but you do not need to make specific changes to the file. OL 9.2 is compiled with all the settings required to recognize and correctly manage ONTAP LUNs.
...
The Oracle Linux 9.2 OS is compiled to recognize ONTAP LUNs and automatically set all configuration parameters correctly for both ASA and non-ASA configurations. You can further optimize performance for your host configuration with the following recommended settings.
...
NOTE: To configure Oracle Linux 9.2 Red Hat Enterprise Kernel (RHCK), use the link:hu_rhel_92.html#recommended-settings[recommended settings] for Red Hat Enterprise Linux (RHEL) 9.2.
...
The Oracle Linux 9.2 with ONTAP release has the following known issues:
