...
. Verify that the `/etc/multipath.conf` exists. 
+
If the reference `/etc/multipath.conf` files doesn't have the NetApp device specific configuration parameters, add these parameters by using the following customized `.conf drop-in` file. You can find this file under `/etc/multipath/conf.d/`:
+
NOTE: Don't make changes to the `/etc/multipath.conf` file because the file contents might be overwritten during a system update.
+
.Show file contents
[%collapsible]
====
...
blacklist {
	wwid <Insert the LUN wwid string>
...
----
====
. The first time you configure multipath, you might need to enable and start the multipath services to load the recommended settings: 
+
[source,cli]
----
...
. Each time you boot the host, the  `/etc/multipath/conf.d/custom.conf` file automatically loads the NetApp recommended host multipath parameters as the default settings. 
+
