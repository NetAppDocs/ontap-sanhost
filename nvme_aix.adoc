...
.About this task 
You can use the following support and features with the NVMe/FC host configuration for an AIX host with ONTAP. You should also review the known limitations before starting the configuration process.
...
** You do not need a separate Path Control Module (PCM), such as Host Utilities for AIX SCSI Microsoft Multipath I/O (MPIO) support, for NVMe devices.
...
* Known limitations: 
The following limitations apply to NVMe/FC host configuration for AIX with ONTAP.

** Qlogic/Marvel 32G FC Host Bus Adapters (HBAs) on an AIX host does not support NVMe/FC.
...
+
The following table lists the HBAs assigned to the AIX Logical Partition (AIX LPAR) or the physical stack:
+
...
|===
|Host OS	|Power Arch	|Power FW version	|Mode	|Comments
|VIOS/PowerVM 3.1.4.21	|Power10	|FW 1010 or later	|Virtual stack	|Support begins from AIX 7.3 TL1 SP2 for VIOC
|===

== Verify multipathing

IBM MPIO (Multi Path I/O) is used for NVMe multipathing and is provided by default when you install the AIX OS.  
...
. The NVMe/FC protocol support is enabled by default in the physical FC but disabled in the Virtual Fibre Channel (VFC) on Virtual I/O Server (VIOS).
+
...
You need to verify that the host correctly reflects ONTAP namespaces:
----
