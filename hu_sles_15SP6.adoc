...
[.lead]
You can use the ONTAP SAN host configuration settings to configure SUSE Linux Enterprise Server (SLES) 15 SP6 with ONTAP as the target. 
...
For SLES 15 SP6, the `/etc/multipath.conf` file must exist. You don't need to make changes to this file because SLES 15 SP6 is compiled with all the settings required to recognize and correctly manage ONTAP LUNs.
...
The SLES 15 SP6 OS is compiled to recognize ONTAP LUNs and automatically set all configuration parameters correctly for both ASA and non-ASA configurations. You can further optimize performance for your host configuration with the following recommended settings.
