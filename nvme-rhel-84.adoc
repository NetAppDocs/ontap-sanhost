...
This document describes how to configure NVMe-oF hosts for Red Hat Enterprise Linux (RHEL) 8.4. For additional support and feature information, refer to link:hu-nvme-index.html[NVME-oF Overview^]. The following are known limitations for RHEL 8.4:
...
* On RHEL 8.4 hosts, NVMe/TCP is available as a technology preview due to known issues.
...
. Install RHEL 8.4 on the server. After the installation is complete, verify that you are running the specified RHEL 8.4 kernel: 
+
[source,cli]
----
uname -r
