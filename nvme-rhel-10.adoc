...
This document describes how to configure NVMe-oF hosts for Red Hat Enterprise Linux (RHEL) 10.0. For additional support and feature information, refer to link:hu-nvme-index.adoc[NVME-oF Overview^]. The following are known limitations for RHEL 10.0:
...
You can configure your host to use SAN booting to simplify deployment and improve scalability, see link:nvme-enable-SAN-booting.adoc[Enable SAN booting^].
...
You can increase the `lpfc` value to issue 1MB I/O requests on the Broadcome NVMe/FC host, see  link:nvme-enable-broadcom-1mb-size.adoc[Enable 1MB I/O^].
