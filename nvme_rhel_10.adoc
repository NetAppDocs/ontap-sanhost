...
* Features available:
** Beginning with ONTAP 9.12.1, support for secure in-band authentication is introduced for NVMe-oF. You can use secure in-band authentication for NVMe-oF with RHEL 10.
** RHEL 10 enables in-kernel NVMe multipath for NVMe namespaces by default, removing the need for explicit settings.
...
You can use the following procedure to validate the minimum supported RHEL 10 software versions.
...
NOTE: The native inbox qla2xxx driver included in the RHEL 10 GA kernel has the latest fixes. These fixes are essential for ONTAP support. 
...
Beginning with ONTAP 9.12.1, secure in-band authentication is supported over NVMe/TCP and NVMe/FC between a RHEL 10 host and an ONTAP controller.
...
No known issues exist for the NVMe-oF host configuration on RHEL 10 with ONTAP release.
