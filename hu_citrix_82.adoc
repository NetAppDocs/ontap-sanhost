...
For Citrix Hypervisor 8.2  the `/etc/multipath.conf` file must exist, but you do not need to make specific changes to the file. Citrix Hypervisor 8.2 is compiled with all settings required to recognize and correctly manage ONTAP LUNs.
...
There are no known issues for the FC host configuration for Citrix Hypervisor 8.2 with ONTAP release.
