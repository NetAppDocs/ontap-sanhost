...
keywords: nvme, oracle linux, 10.1, host configuration
summary: NVMe-oF Host Configuration for Oracle Linux 10 with ONTAP
---
= Configure Oracle Linux 10.1 with NVMe-oF for ONTAP storage 
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
NVMe-oF with Oracle Linux 10.1 has the following known limitation:
...
Use the following procedure to validate the minimum supported Oracle Linux 10.1 software versions.

.Steps

. Install Oracle Linux 10.1 on the server. After the installation is complete, verify that you are running the specified Oracle Linux 10.1 kernel. 
+
[source,cli]
----
uname -r
...
.	On the Oracle Linux 10.1 host, check the `hostnqn` string at `/etc/nvme/hostnqn`: 
+
[source,cli]
----
...
\                                                                                                                                                                               +- nvme4 *tcp* traddr=192.168.20.28,+- nvme4 *tcp* traddr=192.168.20.28,trsvcid=4420,host_traddr=192.168.20.21,src_addr=192.168.20.21 *live optimized*                                                
...
Secure in-band authentication is supported over NVMe/TCP between an Oracle Linux 10.1 host and an ONTAP controller.
...
//2026 Mar 11, ONTAPDOC-3774 Oracle Linux 10.1 release
