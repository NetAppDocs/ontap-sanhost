...
For Oracle Linux (OL) 8.7 the `/etc/multipath.conf` file must exist, but you do not need to make specific changes to the file. OL 9.0 is compiled with all settings required to recognize and correctly manage ONTAP LUNs.
...
NOTE: To configure Oracle Linux 9.0 RedHat Enterprise Kernel (RHCK), use the link:hu_rhel_86.html#recommended-settings[recommended settings] for Red Hat Enterprise Linux (RHEL) 9.0.
