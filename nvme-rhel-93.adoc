...
This document describes how to configure NVMe over Fabrics (NVMe-oF) hosts for RHEL 9.3. For additional support and feature information, refer to link:hu-nvme-index.html[NVME-oF Overview^]. The following are known limitations for RHEL 9.3:
...
Use the following procedure to validate that your RHEL 9.3 system meets the software requirements and verify the NVMe package installations and host configuration.
...
[NOTE]
====
Beginning with RHEL 9.3, the setting for the NVMe/TCP `ctrl_loss_tmo timeout` is automatically set to "off". As a result:
...
With RHEL 9.3, the `nvmefc-boot-connections.service` and `nvmf-autoconnect.service` boot services included in the NVMe/FC `nvme-cli` package are automatically enabled when the system boots. 
