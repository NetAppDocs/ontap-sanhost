...
This document describes how to configure NVMe-oF hosts for Red Hat Enterprise Linux (RHEL) 8.7. For additional support and feature information, refer to link:hu-nvme-index.html[NVME-oF Overview^]. The following are known limitations for RHEL 8.7:
...
* On RHEL 8.7 hosts, NVMe/TCP is available as a technology preview due to known issues.
...
You can use the following procedure to validate the minimum supported RHEL 8.7 software versions.

.Steps

. Install RHEL 8.7 on the server. After the installation is complete, verify that you are running the specified RHEL 8.7 kernel: 
+
[source,cli]
----
uname -r
----
+
The following example shows a RHEL 8.7 kernel version:
+
----
...
+
----
The driver version included in the RHEL 8.7 kernel.
----
