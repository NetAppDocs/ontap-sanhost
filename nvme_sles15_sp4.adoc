...
You can use the following procedure to enable NVMe services.

Between the two NVMe/FC boot services included in the `nvme-cli` package, _only_ `nvmefc-boot-connections.service` is enabled to start during system boot;  `nvmf-autoconnect.service` is not enabled. Therefore, you need to manually enable `nvmf-autoconnect.service` to start during system boot.
...
. Reboot the host. 
. Verify that `nvmf-autoconnect.service` and `nvmefc-boot-connections.service` are running after system boot up. 
+
*Example*
+
----
...
+
[NOTE]
NetApp recommends setting the `ctrl-loss-tmo` option to `-1` so that the NVMe/TCP initiator attempts to try to reconnect indefinitely in the event of a path loss.
...
---------------- ------------------------------------------------------------------------------------
nvme-subsys0     nqn.1992-08.com.netapp:sn.0501daf15dda11eeab68d039eaa7a232:subsystem.unidir_dhchap               

Controllers
--------------
nvme0, nvme1, nvme2, nvme3

Device   SN                   MN                                       FR       TxPort 
-------- -------------------- ---------------------------------------- -------- ------ 
nvme0    81LGgBUqsI3EAAAAAAAE NetApp ONTAP Controller                  FFFFFFFF tcp    
nvme1    81LGgBUqsI3EAAAAAAAE NetApp ONTAP Controller                  FFFFFFFF tcp    
nvme2    81LGgBUqsI3EAAAAAAAE NetApp ONTAP Controller                  FFFFFFFF tcp    
nvme3    81LGgBUqsI3EAAAAAAAE NetApp ONTAP Controller                  FFFFFFFF tcp    

Asdress        Subsystem    Namespaces
-------------- ------------ ----------------
traddr=192.168.2.214,trsvcid=4420,host_traddr=192.168.2.14 nvme-subsys0 nvme0n1
traddr=192.168.2.215,trsvcid=4420,host_traddr=192.168.2.14 nvme-subsys0 nvme0n1
traddr=192.168.1.214,trsvcid=4420,host_traddr=192.168.1.14 nvme-subsys0 nvme0n1
traddr=192.168.1.215,trsvcid=4420,host_traddr=192.168.1.14 nvme-subsys0 nvme0n1

Device       Generic      NSID       Usage                      
------------ ------------ ---------- -------------------------- 
/dev/nvme0n1 /dev/ng0n1   0x1          1.07  GB /   1.07  GB      

Format           Controllers
---------------- ----------------
4 KiB +  0 B   nvme0, nvme1, nvme2, nvme3
...
---------------- ------------------------- ---------------------------------------
/dev/nvme0n1     vs_CLIENT114              /vol/CLIENT114_vol_0_10/CLIENT114_ns10             

NSID UUID                                   Size
---- -------------------------------------- ---------
1    c6586535-da8a-40fa-8c20-759ea0d69d33   1.07GB
...
Beginning with ONTAP 9.11.1, you can create a persistent discovery controller (PDC) for your SLES 15 SP4 host. A PDC is required to automatically detect NVMe subsystem add or remove scenarios and changes to the discovery log page data. You can use the following procedure to create a PDC.
...
.JSON file
--
You can use the config.json file in conjunction with `nvme connect-all` command when multiple NVMe subsystems are available on the ONTAP controller configuration. 

You can generate the JSON file using -o option. Refer to the man pages for more syntax options.
