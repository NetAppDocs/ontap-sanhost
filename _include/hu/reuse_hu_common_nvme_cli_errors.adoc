...
* If the `nvme_info` command output doesn't display the target LIFs as shown example above, check the `/var/log/messages` and `dmesg` command outputs for any suspicious NVMe/FC failures, and report or fix accordingly.
