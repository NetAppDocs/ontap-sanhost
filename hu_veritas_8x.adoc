...
= Using Veritas Infoscale 8 for Linux with NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:nofooter:
...
== Introduction
This document provides guidance on ONTAP SAN host configuration settings for the Veritas Infoscale storage foundation 8 series release for Red Hat Enterprise Linux and Oracle Linux (RHCK based) platforms with FC, FCoE, and iSCSI protocols.
