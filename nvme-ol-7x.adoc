...
summary: Setting up VMe/FC Host Configuration for Oracle Linux 7.x with ONTAP, with examples
---
...
Learn how to configure NVMe over Fabrics (NVMe-oF) hosts for Oracle Linux 7.x. For more support and feature information, see link:nvme-ol-supported-features.html[Oracle Linux ONTAP support and features].
...
. On the Oracle Linux 7.x host, check the `hostnqn` string at `/etc/nvme/hostnqn`: 
+
[source,cli]
----
