...
NVME/FC is supported on ONTAP 9.7 or later.
...
*	Beginning with ESXi 7.0 update 3 and above, NVMe/FC feature is added and is supported from ONTAP 9.9.1.
*	Beginning with ESXi 7.0 and above, HPP (high performance plugin) is the default plugin for NVMe devices.
...
.	Check whether the configuration is supported with required driver/firmware by referring to link:https://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix] NetApp Interoperability Matrix.

.	Set the lpfc driver parameter `lpfc_enable_fc4_type=3` for enabling NVMe/FC support in the `lpfc` driver.
. Reboot the host.

NOTE:	This parameter is set by default for the LPe35000-series adapters. You must perform the following step to set it manually for LPe32000-series and LPe31000-series adapters.
...
.	Check whether configuration is supported with required driver/firmware by referring to  link:https://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix] NetApp Interoperability Matrix.
...
.	Check whether the nvme  is enabled on the adapter.
+
----
