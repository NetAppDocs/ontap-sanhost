...
ONTAP reports an MDTS (Max Data Transfer Size) of 8 MB in the Identify Controller data which means the maximum I/O request size should be up to 1 MB. However, to issue I/O requests of size 1 MB for the Broadcom NVMe/FC host, the `lpfc_sg_seg_cnt` parameter should also be increased to 256 MB from the default value of 64 MB. Use the following instructions to do so:

include::_include/nvme/reuse_nvme_enabling_broadcom_1mb_size.adoc[]

[NOTE] 
This step is not applicable for Qlogic NVMe/FC hosts.
...
|On Oracle Linux 8.7 NVMe over Fabrics (NVMe-oF) hosts, you can use the `nvme discover -p` command to create Persistent Discovery Controllers (PDCs). When this command is used, only one PDC should be created per initiator-target combination. However, if you are running ONTAP 9.10.1 and Oracle Linux 8.7 with an NVMe-oF host, a duplicate PDC is created each time `nvme discover -p` is executed. This leads to unnecessary usage of resources on both the host and the target.
