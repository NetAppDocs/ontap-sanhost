...
NVMe/FC is supported on ONTAP 9.6 or later for Oracle Linux 7.7. The Oracle Linux 7.7 host can run both NVMe and SCSI traffic through the same fibre channel initiator adapter ports. See the link:https://hwu.netapp.com/Home/Index[Hardware Universe^] for a list of supported FC adapters and controllers. For the current list of supported configurations see the link:https://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix^].
...
.	Verify that you are using the recommended Broadcom lpfc firmware, native inbox driver and outbox auto-connect package versions.  For a list of supported versions, see the link:https://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix^].
+
----
