...
NVMe-oF (including NVMe/FC and NVMe/TCP) is supported with Oracle Linux 8.4 with Asymmetric Namespace Access (ANA) required for surviving storage failovers (SFOs) on the ONTAP array. ANA is the ALUA equivalent in the NVM-oF environment, and is currently implemented with in-kernel NVMe Multipath. This document contains the details for enabling NVMe-oF with in-kernel NVMe Multipath using ANA on OL 8.4 and ONTAP as the target.
...
*	With Oracle Linux 8.4, nvme-fc auto-connect scripts are included in the native nvme-cli package. So one may rely on these native auto-connect scripts itself instead of having to install the external vendor provided outbox auto-connect scripts.

*	With Oracle Linux 8.4, a native udev rule is already provided as part of the nvme-cli package which enables round-robin load balancing for NVMe multipath. So one need not manually create this rule any more.
...
*	With Oracle Linux 8.4, the NetApp plugin in the native nvme-cli is capable of displaying ONTAP details as well for ONTAP namespaces.

== Configuration requirements

Please refer to the link:https://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix (IMT)^]  for exact details regarding supported configs and current list of supported configurations.

== Enable NVMe/FC

.Steps

. Install Oracle Linux 8.4 GA on the server. After the installation is complete, verify that you are running the specified Oracle Linux 8.4 GA kernel. See the link:https://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix^] for the most current list of supported versions.
+
----
# uname -r
...
----
             ...
             vs_coexistance_2
            nvme_1
            nqn.2014-08.org.nvmexpress:uuid:753881b6-3163-46f9-8145-0d1653d99389
----
+
...
+
[NOTE] 
If one intends to run both NVMe & SCSI traffic on the same Oracle Linux 8.4 co-existent host, it is recommended to use in-kernel NVMe multipath for ONTAP namespaces and dm-multipath for ONTAP LUNs respectively. And that also means the ONTAP namespaces should be blacklisted in dm-multipath to prevent dm-multipath from claiming these namespace devices. This can be done by adding the enable_foreign setting to the /etc/multipath.conf:
+
----
...
. After the host reboot, verify that lpfc_sg_seg_cnt is set to 256.
+
----
...
.	The native inbox qla2xxx driver included in the OL 8.4 GA kernel has the latest upstream fixes, essential for ONTAP support. Verify that you are running the supported adapter driver and firmware versions:
+
----
...
|1479047	|RHEL 8.x NVMe-oF hosts create duplicate Persistent Discovery Controllers	|On NVMe over Fabrics (NVMe-oF) hosts, you can create Persistent Discovery Controllers (PDCs) by providing the `-p` option to the `nvme discover` command. For a specified initiator-target combination, the creation of one PDC is expected with each invocation of the `nvme discover` command. However, beginning with ONTAP 9.10.1, Red Hat Enterprise Linux (RHEL) 8.x NVMe-oF hosts end up creating duplicate PDCs with each invocation of the `nvme discover` command with the `-p` option. This leads to wastage of resources on both the host and target. 	|2087000
|===
