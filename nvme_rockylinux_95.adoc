...
= NVMe-oF host configuration for Rocky Linux 9.5 with ONTAP
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
* Features available:
** Beginning with ONTAP 9.12.1, support for secure in-band authentication is introduced for NVMe-oF. You can use secure in-band authentication for NVMe-oF with Rocky Linux 9.5.
...
You can use the following procedure to validate the minimum supported Rocky Linux 9.5 software versions.
...
. On the Rocky Linux 9.5 host, check the hostnqn string at `/etc/nvme/hostnqn`:
+
----
...
Beginning with ONTAP 9.12.1, secure in-band authentication is supported over NVMe/TCP and NVMe/FC between a Rocky Linux 9.5 host and an ONTAP controller.
...
cat /etc/nvme/hostnqn
----
. Generate the dhchap key for the Rocky Linux 9.5 host.
+
