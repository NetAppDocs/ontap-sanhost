...
|CONTAPEXT-2693	
|ESXi 8.0U3 NVMe/TCP fails when SnapMirror active sync CG enters Out-of-Sync state
|ESXi 8.0U3 environments configured with NVMe/TCP and SnapMirror active sync on NVMe namespaces might fail when the Consistency Group (CG) relationship status transitions to an Out-of-Sync state. The CG can move from the InSync state to the Out-of-Sync state due to various workflows, such as an automatic unplanned failover or a disruption to the inter-cluster connection. For more information, refer to  VMware Bug ID: TR1049746.
...
link:https://docs.netapp.com/us-en/ontap/snapmirror-active-sync/interoperability-reference.html[Learn aabout SnapMirror active sync interoperability in ONTAP^]
