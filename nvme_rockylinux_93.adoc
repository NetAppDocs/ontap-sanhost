...
= NVMe-oF host configuration for Rocky Linux 9.3 with ONTAP
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
[.lead]
NVMe over Fabrics (NVMe-oF), including NVMe over Fibre Channel (NVMe/FC) and other transports, is supported with Rocky Linux 9.3 with Asymmetric Namespace Access (ANA). In NVMe-oF environments, ANA is the equivalent of ALUA multipathing in iSCSI and FC environments and is implemented with in-kernel NVMe multipath. 

The following support is available for NVMe-oF host configuration for Rocky Linux 9.3 with ONTAP:
...
Rocky Linux 9.3 has in-kernel NVMe multipath enabled for NVMe namespaces by default; therefore, there is no need for explicit settings.
...
You can use the following procedure to validate the minimum supported Rocky Linux 9.3 software versions.

.Steps

. Install Rocky Linux 9.3 on the server. After the installation is complete, verify that you are running the specified Rocky Linux 9.3 kernel: 
+
----
# uname -r
----
...
. On the Rocky Linux 9.3 host, check the hostnqn string at `/etc/nvme/hostnqn`:
+
----
...
----------- --------------- ----------------------------------------------------------
vs_nvme147   rockylinux_147_LPe32002    nqn.2014-08.org.nvmexpress:uuid:060fd513-83be-4c3e-aba1-52e169056dcf
----
+
[NOTE]
...
The native inbox qla2xxx driver included in the Rocky Linux 9.3 GA kernel has the latest fixes. These fixes are essential for ONTAP support. 
...
[subs=+quotes]
----
nvme-subsys4 - NQN=nqn.1992-08.com.netapp:sn.e80cc121ca6911ed8cbdd039ea165590:subsystem.rockylinux_
147_LPE32002
\
...
nvme-subsys1 - NQN=nqn.1992- 08.com.netapp:sn. bbfb4ee8dfb611edbd07d039ea165590:subsystem.rockylinux_tcp_95
...
There are no known issues for the NVMe-oF host configuration for Rocky Linux 9.3 with ONTAP release.
