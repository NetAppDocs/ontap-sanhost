...
summary: How to Configure NVMe-oF Host for RHEL 8x with ONTAP
---
= Configure RHEL 8x for NVMe-oF with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
Learn how to configure NVMe over Fabrics (NVMe-oF) hosts for RHEL 8x. For more support and feature information, see link:nvme-rhel-supported-features.html[RHEL ONTAP support and features].

NVMe-oF with RHEL 8x has the following known limitations:

* SAN booting using the NVMe-oF protocol is not currently supported.
* In-kernel NVMe multipath is disabled by default on NVMe-oF hosts in RHEL 8x; you must enable it manually.
...
. Install RHEL 8x on the server. After the installation is complete, verify that you are running the required RHEL 8x kernel: 
+
[source,cli]
----
uname -r
...
. On the RHEL 8x host, check the `hostnqn` string at `/etc/nvme/hostnqn`:
+
[source,cli]
----
...
|link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047[1479047^]	|RHEL 8x NVMe-oF hosts create duplicate persistent discovery controllers (PDCs)	|On NVMe-oF hosts, you can use the "nvme discover -p" command to create PDCs. When this command is used, only one PDC should be created per initiator-target combination.  However, if you are running RHEL 8x on an NVMe-oF host, a duplicate PDC is created each time "nvme discover -p" is executed. This leads to unnecessary usage of resources on both the host and the target.
|===
