...
[NOTE]
Beginning with RHEL 9.4, the NVMe/TCP `ctrl_loss_tmo` timeout behaviour is set to infinity by default. Therefore, there is no need to configure a specific timeout period. With default behaviour NVMe/TCP will not timeout and will remain connected indefinitely, eliminating the need for setting a timeout for nvme connect/connect-all (argument -l) in the event of path failure.
