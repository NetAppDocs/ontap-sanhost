...
summary: How to Configure NVMe-oF Host for Proxmox VE 8x with ONTAP storage
---
= Configure Proxmox VE 8x for NVMe-oF with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
.About this task 
You can use the following support and features with the NVMe-oF host configuration for Proxmox 8x. You should also review the known limitations before starting the configuration process.
...
* Features available:
** Native NVMe multipathing is always enabled for Proxmox 8x. 
* Known limitations: 
...
Use the following procedure to validate the minimum supported Proxmox 8x software versions.

.Steps

. Install Proxmox 8x on the server. After the installation is complete, verify that you are running the specified Proxmox 8x kernel: 
+
[source,cli]
----
uname -r
...
With Proxmox 8x, the `nvmefc-boot-connections.service` and `nvmf-autoconnect.service` boot services included in the NVMe/FC `nvme-cli` package are automatically enabled when the system boots. 
