...
* NVMe in-band authentication is supported beginning with SLES15 SP4. 
...
In-kernel NVMe multipath is already enabled by default on SLES hosts such as SLES15 SP4. Therefore, no additional setting is required here. 
...
[NOTE]
====
* If the `rpm -ivh` command fails with a message that a newer package has already been installed, then proceed to install the following using the --oldpackage option, for example:
+
`# rpm -ivh kernel-default-5.14.21-150400.24.21.2.x86_64.rpm --oldpackage`

* To make the kernel a default entry in the grub bootloader, execute the following commands.
+
----
...
During reboot, the host will now choose the above kernel as the default grub entry.
====
...
. Upgrade to the latest available SLES15 SP4 `libnvme1-1.0-150400.3.12.1` and `nvme-cli-2.0-150400.3.12.1` MU associated packages.
+
----
# rpm -qa|grep nvme
...
. Verify that in-kernel NVMe multipath is indeed enabled by checking as follows:
+
----
...
------------------- ---------------------------- -------------------------------
/dev/nvme1n1  /dev/ng1n1     814vWBNRwfBGAAAAAAAB       NetApp ONTAP Controller                     
...
Node           Generic            SN                        Model      
----------------------------------------------- ----------------------------------------
/dev/nvme0n1    /dev/ng0n1        81CYrBQuTHQFAAAAAAAC       NetApp ONTAP Controller        
...
[NOTE]
====
There are slight changes to the `nvme list` and `nvme list-subsys` command outputs compared to the outputs seen from previous OS releases such as SLES15 SP3:

* `nvme list` has a new column `Generic` to denote the respective NVMe generic device.
* `nvme list-subsys` has a comma separating the `trsvcid` and `host-traddr` fields.
====

== Discovery Hardening - TP 8013 and TP 8014 (with ONTAP 9.11.1)

Technical Proposals (TP) 8013 specifies the usage of unique subsystem NQNs (in addition to the well known discovery NQN) for discovery subsystems. TP-8013 has been implemented beginning with ONTAP 9.11.1. This feature is implemented in the new `libnvme/nvme-cli v2.x` package in SLES15 SP4. Therefore, a device can now connect to the specified unique subsystem NQN of the specified ONTAP discovery controller.
...
For the Entry Flags, ONTAP sets EPCSD (Explicit Persistent Connection Support for Discovery) to 1 and 0 for Discovery subsystems and I/O subsystems respectively. ONTAP also sets DUPRETINFO (Duplicate Returned Information) to 1 and 0 for Discovery subsystems and I/O subsystems respectively. The new `libnvme/nvme-cli v2.x` package in SLES15 SP4 can consume these settings correctly and connect accordingly. 
...
. Validate that the specified PDC is created by connecting to its unique subsystem NQN (not the well known NQN) in ONTAP:
+
----
...
----
+
For example, to generate a random dhchap-key using hmac set to 3 (such as SHA-512) along with the host nqn:
+
----
...
----
+
This might be used as a secret on the host and the target for a specified subsystem during the nvme connect with the following dhchap options:
+
----
...
If no secret is specified in the Linux `nvme-cli dhchap` option during the connect operation, it will attempt to locate the default key at `/etc/nvme/hostkey`. If not found, authentication will not be attempted.

=== In-band Authentication using CLI

Following procedure is an example of using In-band Authentication using bidirectional connect through the CLI.
...
. On the host, proceed with a bidirectional connect specifying both these keys.
+
----
...
. Now run `connect-all` to the config JSON file (automatically the discovery options from the `/etc/nvme/discovery.conf` file will be selected):
+
----
