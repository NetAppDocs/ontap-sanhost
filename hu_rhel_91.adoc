...
|1508554   	|NetApp Linux Host Utilities cli needs add-on library package dependencies to support Emulex HBA adapter discovery	|In RHEL 9.1, NetApp Linux SAN host utilities CLI command - `sanlun fcp show adapter -v` fails because the library package dependencies to support HBA discovery cannot be found. |N/A
|1502137	|FC transport layer timeout parameter "dev_loss_tmo" for remote SCSI target ports reports a negative value	|dev_loss_tmo timeout parameter represents the default number of seconds that the FC transport should insulate to prevent the loss of a remote port.
...
However in RHEL 8.7, sysfs filesystem returns a negative value (-1) for the dev_loss_tmo timeout parameter for SCSI devices.	|2126324
|===
