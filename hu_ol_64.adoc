...
NOTE: To configure Oracle Linux 6.4 Red Hat Enterprise Kernel (RHCK), use the link:hu_rhel_64.html#recommended-settings[recommended settings] for Red Hat Enterprise Linux (RHEL) 6.4.
...
| link:htthttps://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=715217[715217^] | 	Delay in path recovery on OL6.4 or OL5.9 hosts with UEK2 might result in delayed I/O resumption on controller or fabric faults | When a controller fault (storage failover or giveback, reboots and so on) or a fabric fault (FC port disable or enable) occurs with I/O on Oracle Linux 6.4 or Oracle Linux 5.9 hosts with UEK2 Kernel, the path recovery by DM-Multipath takes a long time (4mins. to 10 mins).
...
NOTE: For Oracle Linux (Red Hat compatible kernel) known issues, see the link:hu_rhel_64.html#known-problems-and-limitations[known issues] for Red Hat Enterprise Linux (RHEL) 6.4.
