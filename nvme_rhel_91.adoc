...
NVMe-oF (including NVMe/FC and NVMe/TCP) is supported with RHEL 9.1 with Asymmetric Namespace Access (ANA), this is required for surviving storage failovers (SFOs) on the ONTAP array. ANA is the asymmetric logical unit access (ALUA) equivalent in the NVM-oF environment, and is currently implemented with in-kernel NVMe Multipath. This document contains the details for enabling NVMe-oF with in-kernel NVMe Multipath using ANA on RHEL 9.1 and ONTAP as the target.

include::_include/hu/reuse_hu_cloud_note.adoc[]

== Features

*	Beginning with RHEL 9.1, NVMe/TCP is no longer a technology preview feature (unlike RHEL 8), but a fully supported enterprise feature itself.
*	Beginning with RHEL 9.1, in-kernel NVMe multipath is enabled for NVMe namespaces by default, without the need for explicit settings (unlike RHEL 8).


== Limitations

*	Unlike NVMe/FC, NVMe/TCP has no auto-connect functionality. This is reflected in  two major limitations on the Linux host:
**	No auto-reconnect after paths are reinstated - NVMe/TCP cannot automatically reconnect to a path that is reinstated beyond the default ctrl-loss-tmo time of 10 minutes following a path down.
**	No auto-connect during host bootup - NVMe/TCP cannot automatically connect during host bootup.You should set the retry period for failover events to at least 30 minutes to prevent timeouts. You can increase the retry period by increasing the value of the ctrl_loss_tmo timer.
...
.	Install RHEL 9.1 on the server. After the installation is complete, verify that you are running the specified RHEL 8.7 kernel. See the link:https://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix^] for the most current list of supported versions.
...
The native inbox `qla2xxx` driver included in the RHEL 8.6 kernel has the latest upstream fixes which are essential for ONTAP support.
...
Unlike NVMe/FC, NVMe/TCP has no auto-connect functionality. This is reflected in two major limitations on the Linux NVMe/TCP host:

*	No auto-reconnect after paths get reinstated – NVMe/TCP cannot automatically reconnect to a path that is reinstated beyond the default ctrl-loss-tmo time of 10 minutes following a path down event.
*	No auto-connect during host bootup – NVMe/TCP cannot automatically connect during host bootup.
...
nvme discover -a 192.168.166.21 -t tcp -w 192.168.166.5  -p
...
portid:  0
trsvcid: 8009
subnqn:  nqn.1992-08.com.netapp:sn.bf0691a7c74411ec8187d039ea14107d:discovery
traddr:  192.168.167.20
...
portid:  0
trsvcid: 4420
subnqn:  nqn.1992-08.com.netapp:sn.bf0691a7c74411ec8187d039ea14107d:subsystem.rhel_tcp_133
traddr:  192.168.167.20
...
----
nvme discover -a 192.168.166.21 -t tcp -w 192.168.166.5  -p
nvme discover -a 192.168.167.21 -t tcp -w 192.168.167.5  -p 
...
+
----
nvme connect-all -t tcp -w 192.168.166.5 -a 192.168.166.20 -l 1800
nvme connect-all -t tcp -w 192.168.167.5 -a 192.168.167.21 -l 1800
nvme connect-all -t tcp -w 192.168.166.5 -a 192.168.166.21 -l 1800
...
|1479047	|RHEL 8.x and 9.0 NVMe-oF hosts create duplicate Persistent Discovery Controllers	|On NVMe over Fabrics (NVMe-oF) hosts, you can create Persistent Discovery Controllers (PDCs) by passing the -p option to the "nvme discover" command. For a given initiator-target combination, the creation of one PDC is expected with each invocation of the "nvme discover" command. However, beginning with ONTAP 9.10.1, Red Hat Enterprise Linux (RHEL) 8.x and 9.0 NVMe-oF hosts end up creating duplicate PDCs with each invocation of the "nvme discover" command with the -p option. This leads to wastage of resources on both the host and the target. 	|2087000
|===
