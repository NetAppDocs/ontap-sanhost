---
sidebar: sidebar
permalink: nvme_rockylinux_88.html
keywords: nvme, linux, rocky linux, enterprise
summary: How to Configure NVMe-oF Host for Rocky Linux 8.8 with ONTAP
---

= NVMe-oF host configuration for Rocky Linux 8.8 with ONTAP
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
[.lead]
NVMe over Fabrics (NVMe-oF), including NVMe over Fibre Channel (NVMe/FC) and other transports, is supported with Rocky Linux 8.8 with Asymmetric Namespace Access (ANA). In NVMe-oF environments, ANA is the equivalent of ALUA multipathing in iSCSI and FC environments and is implemented with in-kernel NVMe multipath. 

The following support is available for the NVMe-oF host configuration for Rocky Linux 8.8 with ONTAP:
...
* In-kernel NVMe multipath is disabled by default for Rocky Linux 8.8 NVMe-oF hosts. Therefore, you need to enable it manually.
* On Rocky Linux 8.8 hosts, NVMe/TCP is a technology preview feature due to open issues.
...
. Install Rocky Linux 8.8 on the host server. 
. After the installation is complete, verify that you are running the specified Rocky Linux 8.8 kernel. 
+
----
# uname -r
----
...
----------- --------------- ----------------------------------------------------------
vs_nvme161 rockylinux_161_LPe32002    nqn.2014-08.org.nvmexpress:uuid:f6517cae-3133-11e8-bbff-7ed30aef123f
----
+
[NOTE]
...
--
The native inbox qla2xxx driver included in the Rocky Linux 8.8 GA kernel has the latest upstream fixes. These fixes are essential for ONTAP support. 
...
----
nvme-subsys3 - NQN=nqn.1992-08.com.netapp:sn.ab4fa6a5ba8b11ecbe3dd039ea359e4b:subsystem.rockylinux_161_Lpe32002
\
...
The NVMe-oF host configuration for Rocky Linux 8.8 with ONTAP release has the following known issues:

[cols="20,40,40",options="header"]
|===
|NetApp Bug ID	|Title	|Description

|link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047[1479047]	|Rocky Linux 8.8 NVMe-oF hosts create duplicate persistent discovery controllers	|On NVMe over Fabrics (NVMe-oF) hosts, you can use the "nvme discover -p" command to create Persistent Discovery Controllers (PDCs). When this command is used, only one PDC should be created per initiator-target combination.  However, if you are running Rocky Linux 8.8 on an NVMe-oF host, a duplicate PDC is created each time "nvme discover -p" is executed. This leads to unnecessary usage of resources on both the host and the target.
|===
