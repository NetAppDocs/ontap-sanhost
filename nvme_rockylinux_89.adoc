...
= NVMe-oF host configuration for Rocky Linux 8.9 with ONTAP
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
[.lead]
NVMe over Fabrics (NVMe-oF), including NVMe over Fibre Channel (NVMe/FC) and other transports, is supported with Rocky Linux 8.9 with Asymmetric Namespace Access (ANA). In NVMe-oF environments, ANA is the equivalent of ALUA multipathing in iSCSI and FC environments and is implemented with in-kernel NVMe multipath. 

The following support is available for NVMe-oF host configuration for Rocky Linux 8.9 with ONTAP:
...
* On Rocky Linux 8.9 hosts, NVMe/TCP is a technology preview feature due to open issues.
...
. Install Rocky Linux 8.9 on the host server. 
. After the installation is complete, verify that you are running the specified Rocky Linux 8.9 kernel: 
+
----
# uname -r
----
...
----------- --------------- ----------------------------------------------------------
vs_nvme101 rockylinux_101_QLe2772    nqn.2014-08.org.nvmexpress: uuid:4c4c4544-0032-3410-8035-b8c04f4c5132
----
+
[NOTE]
...
The native inbox qla2xxx driver included in the Rocky Linux 8.9 GA kernel has the latest upstream fixes. These fixes are essential for ONTAP support.
...
[subs=+quotes]
----
nvme-subsys0 - NQN=nqn.1992-08.com.netapp:sn.8e501f8ebafa11ec9b99d039ea359e4b:subsystem.rockylinux_163_Qle2742 
