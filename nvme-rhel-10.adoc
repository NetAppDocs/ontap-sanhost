...
This document describes how to configure NVMe-oF hosts for RHEL 10.0. For additional support and feature information, refer to link:hu-nvme-index.html[NVME-oF Overview^]. The following are known limitations for RHEL 10.0:
...
== Step 2: Verify software versions and NVMe configuration

Use the following procedure to validate that your RHEL 10.0 system meets the software requirements and verify the NVMe package installations and host configuration.
...
You can enable I/O requests of size 1MB for NVMe/FC configured with a Broadcom adapter, see  link:nvme-enable-broadcom-1mb-size.html[Enable 1MB I/O^].

== Step 5: Verify NVMe boot services 

With RHEL 10.0, the `nvmefc-boot-connections.service` and `nvmf-autoconnect.service` boot services included in the NVMe/FC `nvme-cli` package are automatically enabled when the system boots. 
