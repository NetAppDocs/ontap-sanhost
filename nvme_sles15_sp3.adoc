...
*	There is no sanlun support for NVMe-oF. Therefore, there is no LUHU support for NVMe-oF on SUSE Linux Enterprise Server 15 SP3. You can use the NetApp plug-in included in the native nvme-cli for the same instead. This should support all NVMe-oF transports.
...
In the following example, only 1 initiator port is enabled and connected with two target LIFs as seen in the output:
+
[subs=+quotes]
----
...
The native inbox qla2xxx driver included in the newer SUSE Linux Enterprise Server 15 SP3 MU kernel has the latest upstream fixes, which are essential for ONTAP support.
