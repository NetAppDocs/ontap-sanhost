...
= Using SUSE Linux Enterprise Server 15 SP4 with NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:nofooter:
...
The following example shows how to correct an overridden default. In this case, the `multipath.conf` file defines values for `path_checker` and `no_path_retry` that are not compatible with ONTAP LUNs. If they cannot be removed because of other SAN arrays being still attached to the host, these parameters can be corrected specifically for ONTAP LUNs with a device stanza.
...
There are no known issues for SUSE Linux Enterprise Server 15 SP4.
