...
The Ubuntu 24.04 OS is compiled to recognize ONTAP LUNs and automatically set all configuration parameters correctly for ASA and non-ASA configurations. You can use the following recommended settings to further optimize the performance of your host configuration:
...
=== KVM settings
You can use the <<recommend_settings_2404,recommended settings>> to configure Kernel-based Virtual Machine (KVM) as well. There are no changes required to configure KVM as the LUN is mapped to the hypervisor.
