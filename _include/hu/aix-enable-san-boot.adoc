...
NOTE: AIX requires that only one path be visible to a SAN boot disk until MPIO is enabled. Installing or migrating an AIX boot disk generally requires temporarily restricting the FC zoning for the host to iniclude a single FC initiator and a single FC target. After you install the Host Utilities, including the PCM package, you can reboot the OS to fully enable MPIO. At that point, add the additional FC initiators and FC targets to the zone. Then, run `cfgmgr -v` and `bosboot -a` to update the boot configuration to include all available paths.
