...
This document describes how to configure NVMe over Fabrics (NVMe-oF) hosts for RHEL 8.10. For additional support and feature information, refer to link:hu-nvme-index.html[NVME-oF Overview^]. The following are known limitations for RHEL 8.10:
...
* On RHEL 8.10 hosts, NVMe/TCP is available as a technology preview due to known issues.
...
You can use the following procedure to verify the minimum supported RHEL 8.10 software versions and NVMe configuration.
...
----
+
The following example shows a RHEL 8.10 kernel version:
+
----
...
With RHEL 8.10, the `nvmefc-boot-connections.service` and `nvmf-autoconnect.service` boot services included in the NVMe/FC `nvme-cli` package are automatically enabled when the system boots. 
