...
== Enable NVMe serrvices

Of the two NVMe/FC boot services included in the nvme-cli, only nvmefc-boot-connections.service is enabled to start at boot, but not nvmf-autoconnect.service. So manually enable nvmf-autoconnect.service to start at boot by running:
...
Next reboot the host. And verify that nvmf-autoconnect.service (along with nvmefc-boot-connections.service) are running after bootup. For example,
