...
This document describes how to configure NVMe over Fabrics (NVMe-oF) hosts for Red Hat Enterprise Linux (RHEL) 9.6. For more support and feature information, see link:hu-nvme-index.html[NVME-oF Overview^]. NVMe-oF with RHEL 9.6 has the following known limitations:
...
=== Modify the iopolicy in Udev Rules

For RHEL 9.6, the default iopolicy for NVMe-oF is set to `round-robin`. If you want to change the iopolicy to `queue-depth`, modify the udev rules file as follows:

. Edit the udev rules file. Open the following file in a text editor with root privileges:
+
[source,cli]
----
