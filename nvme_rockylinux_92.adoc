...
NVMe over Fabrics (NVMe-oF), including NVMe over Fibre Channel (NVMe/FC) and other transports, is supported with Rocky Linux 9.2 with Asymmetric Namespace Access (ANA). In NVMe-oF environments, ANA is the equivalent of ALUA multipathing in iSCSI and FC environments and is implemented with in-kernel NVMe multipath. 

The following support is available for the NVMe-oF host configuration for Rocky Linux 9.2 with ONTAP:
...
* Rocky Linux 9.2 has in-kernel NVMe multipath enabled for NVMe namespaces by default, therefore, there is no need for explicit settings.
...
You can use the following procedure to validate the minimum supported Rocky Linux 9.2 software versions.

.Steps

. Install Rocky Linux 9.2 on the server. After the installation is complete, verify that you are running the specified Rocky Linux 9.2 kernel. 
+
----
# uname -r
----
...
. On the Rocky Linux 9.2 host, check the hostnqn string at `/etc/nvme/hostnqn`:
+
----
...
The native inbox qla2xxx driver included in the Rocky Linux 9.2 GA kernel has the latest upstream fixes. These fixes are essential for ONTAP support. 
...
----
nvme-subsys4 - NQN=nqn.1992-08.com.netapp:sn.8763d311b2ac11ed950ed039ea951c46:subsystem.rockylinux_207_LB \
...
nvme-subsys1 - NQN=nqn.1992-08.com.netapp:sn.c680f5bcae1411ed8639d039ea951c46:subsystem.rocky linux_tcp97 \
