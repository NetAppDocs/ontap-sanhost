...
Unlike NVMe/FC, NVMe/TCP lacks two major features:

*	*No auto-connect scripts.* Due to the lack of auto-connect scripts, NVMe/TCP cannot automatically reconnect after a path is reinstated beyond the default period of 10 minutes following the path down period.

*	*Longer retry period following a path down.* By default, NVMe/TCP retries for a period of 10 minutes (30 seconds for NVMe/FC) following a path down. The NVMe controller would be in `connecting` state at all times. When the controller is in `connecting` state, you might encounter errors in command outputs while running the `nvme list` command.

Therefore, you are always expected to manually run a `nvme connect-all` command across all NVMe/TCP target LIFs (across all ONTAP nodes) during configuration. You must run this command even after a host reboot. To comfortably tide over ONTAP failover events such as SFOs, it is advised to set a longer retry period such as, say 30 minutes, by tweaking `ctrl_loss_tmo` parameter. Following are the details:
...
. Verify that the ONTAP NVMe/TCP namespaces properly reflect on the host. For example,
+
----
# nvme list
...
. Verify the NetApp plug-in displays proper values for each ONTAP namespace device. For example,
+
----
# nvme netapp ontapdevices -o column
Device        Vserver        Namespace Path                        NSID
------------- -------------- ------------------------------------- ------------
/dev/nvme0n1    vs_tcp_118   /vol/tcpnvme_118_1_0_0/tcpnvme_118_ns  1
UUID                                  Size
------------------------------------  -------
4a3e89de-b239-45d8-be0c-b81f6418283c  85.90GB
