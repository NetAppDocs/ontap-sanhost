...
|1503468
|nvme list-subsys returns repeated nvme controller list for a given subsystem.	|In RHEL 9.1 'nvme list-subsys' returns nvme controllers with its respective ANA state for all namespaces, that belongs to a given subsystem. 
However the ANA state is a per namespace attribute hence it only makes sense to show it if the user lists the subsystem for a namespace.
Hence 'nvme list-subsys' returning the unique list of nvme controllers associated to a given subsystem would be more ideal.  	|2130106
