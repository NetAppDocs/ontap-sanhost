...
---
= Configure RHEL 8.8 for NVMe-oF with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
This document describes how to configure NVMe over Fabrics (NVMe-oF) hosts for RHEL 8.8. For additional support and feature information, refer to link:hu-nvme-index.html[NVME-oF Overview^]. The following are known limitations for RHEL 8.8:

* SAN booting using the NVMe-oF protocol is not currently supported.
* In-kernel NVMe multipath is disabled by default on NVMe-oF hosts in RHEL 8.8; you must enable it manually.
...
You can use the following procedure to validate the minimum supported RHEL 8.8 software versions.

.Steps

. Install RHEL 8.8 on the server. After the installation is complete, verify that you are running the required RHEL 8.8 kernel: 
+
[source,cli]
----
uname -r
----
+
The following example shows a RHEL 8.8 kernel version:
+
----
...
. On the RHEL 8.8 host, check the `hostnqn` string at `/etc/nvme/hostnqn`:
+
[source,cli]
----
...
|link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047[1479047]	|RHEL 8.8 NVMe-oF hosts create duplicate persistent discovery controllers (PDCs)	|On NVMe-oF hosts, you can use the "nvme discover -p" command to create PDCs. When this command is used, only one PDC should be created per initiator-target combination.  However, if you are running RHEL 8.8 on an NVMe-oF host, a duplicate PDC is created each time "nvme discover -p" is executed. This leads to unnecessary usage of resources on both the host and the target.
|===
