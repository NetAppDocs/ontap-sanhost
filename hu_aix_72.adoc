...
If storage failover (SFO) occurs on both clusters around the same time (concurrent SFO), I/O is expected to fail because the primary path of the LUN might return `LU NOT SUPPORTED`. To avoid I/O failure, you must set the following bootarg on all 4 nodes of both clusters:
...
. Remove the failed paths from the primary cluster for all devices:
+
*Example*
+
----
