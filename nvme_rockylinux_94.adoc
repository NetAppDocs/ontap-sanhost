...
[.lead]
NVMe over Fabrics (NVMe-oF), including NVMe over Fibre Channel (NVMe/FC) and other transports, is supported with Rocky Linux 9.4 with Asymmetric Namespace Access (ANA). In NVMe-oF environments, ANA is the equivalent of ALUA multipathing in iSCSI and FC environments and is implemented with in-kernel NVMe multipath. 

The following support is available for NVMe-oF host configuration for Rocky Linux 9.4 with ONTAP:
...
* Rocky Linux 9.4 has in-kernel NVMe multipath enabled for NVMe namespaces by default; therefore, there is no need for explicit settings.
...
You can use the following procedure to validate the minimum supported Rocky Linux 9.4 software versions.
...
. On the Rocky Linux 9.4 host, check the hostnqn string at `/etc/nvme/hostnqn`:
+
----
...
The native inbox qla2xxx driver included in the Rocky Linux 9.4 GA kernel has the latest fixes. These fixes are essential for ONTAP support. 
...
There are no known issues for the NVMe-oF host configuration for Rocky Linux 9.4 with ONTAP release.
