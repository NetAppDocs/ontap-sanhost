...
QLE2742 FW:v9.28.02 DVR:v10.02.00.106-k
QLE2742 FW:v9.28.02 DVR:v10.02.00.106-k
----
...
+- nvme6 tcp traddr=192.168.9.49,trsvcid=4420,host_traddr=192.168.9.2 live non-optimized
+- nvme7 tcp traddr=192.168.9.48,trsvcid=4420,host_traddr=192.168.9.2 live non-optimized
----
--
====
...
The NVMe-oF host configuration for OL 9.2 with ONTAP release has the following known issues:
...
|The `nvme list-subsys` command should return a unique list of NVMe controllers associated with a given subsystem. In Oracle Linux 9.2, the `nvme list-subsys` command returns NVMe controllers with the respective asymmetric namespace access (ANA) state for all namespaces that belong to a given subsystem. However, it would be useful to display unique NVMe controller entries with the path state if you list the subsystem command syntax for a given namespace because the ANA state is a per-namespace attribute.

|link:https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=17998[17998^]
|1539101 |Oracle Linux 9.2 NVMe-oF hosts fail to create a persistent discovery controller | On Oracle Linux 9.2 NVMe-oF hosts, you can use the `nvme discover -p` command to create Persistent Discovery Controllers (PDCs). When this command is used, one PDC should be created per initiator-target combination. However, if you are running Oracle Linux 9.2 on an NVMe-oF host, PDC creation fails when the `nvme discover -p` command is executed.  
