...
summary: Describes how to use CentOS 7.4 with ONTAP
---

= Using CentOS 7.4 with NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:nofooter:
...
For CentOS 7.4 the /etc/multipath.conf file must exist, but you do not need to make specific changes to the file. CentOS 7.4 is compiled with all settings required to recognize and correctly manage ONTAP LUNs.
...
The CentOS 7.4 OS is compiled to recognize ONTAP LUNs and automatically set all configuration parameters correctly for both ASA and non-ASA configuration.
...
For CentOS (Red Hat compatible kernel) known issues, see the link:hu_rhel_74.html#known-problems-and-limitations[known issues] for Red Hat Enterprise Linux (RHEL) 7.4.
