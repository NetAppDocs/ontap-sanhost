...
In order to make sure the Solaris client applications are non-disruptive when an unplanned site failover switchover occurs in a SnapMirror Business Continuity (SM-BC) environment, the following setting needs to be configured on the Solaris 11.4 host. This setting will override the failover module – `f_tpgs` in order to prevent the code path that detects the contradiction from being executed.
...
"NETAPP  LUN","f_tpgs"
----
. Use `devprop` and `mdb` commands to verify the override parameter has been successfully applied:
+
...
NOTE: `conf` will be added to `svl_fops_name` when `scsi-vhci-failover-override` has been applied.
