...
summary: How to Configure NVMe-oF Host for RHEL 9x with ONTAP
---
= Configure RHEL 9x for NVMe-oF with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
Learn how to configure NVMe over Fabrics (NVMe-oF) hosts for Red Hat Enterprise Linux (RHEL) 9x. For more support and feature information, see link:hu-nvme-index.html[NVME-oF Overview^]. 

*NVMe-oF with RHEL 9x has the following known limitations:*
...
. Install RHEL 9x on the server. After the installation is complete, verify that you are running the required RHEL 9x kernel: 
+
[source,cli]
----
uname -r
...
Beginning with ONTAP 9.12.1, secure in-band authentication is supported over NVMe/TCP between a RHEL 9x host and an ONTAP controller.
...
cat /etc/nvme/hostnqn
----
. Generate the dhchap key for the RHEL 9x host.
+
