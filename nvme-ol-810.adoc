...
[.lead]
NetApp SAN host configurations support the NVMe over Fabrics (NVMe-oF) protocol on Oracle Linux 8.10 with Asymmetric Namespace Access (ANA). 
...
.. Restart the `multipathd` daemon. This allows the new setting to take effect: 
+
