...
|120
|When MPIO support is specified and your server is Windows Server 2022, 2019, 2016, or 2012 R2
|HKLM\SYSTEM\CurrentControlSet\Services
...
\Parameters\PDORemovePeriod
|130
|When MPIO support is specified, except if ONTAP DSM is detected
|HKLM\SYSTEM\CurrentControlSet\Services\mpio
...
\msdsm\Parameters
\DsmMaximumRetryTimeDuringStateTransition
|120 |When MPIO support is specified and your server is Windows Server 2016, 2012 R2, 2012, 2008 R2, or 2008, except if ONTAP DSM is detected
|HKLM\SYSTEM\CurrentControlSet\Services
\msdsm\Parameters
\DsmMaximumStateTransitionTime
|120 |When MPIO support is specified and your server is Windows Server 2016, 2012 R2, 2012, 2008 R2, or 2008, except if ONTAP DSM is detected
...
|"NETAPP LUN", "NETAPP LUN C-Mode" |When MPIO support is specified, except if ONTAP DSM is detected
|HKLM\SYSTEM\CurrentControlSet\Control\Class
\{iSCSI_driver_GUID}\ instance_ID\Parameters
\IPSecConfigTimeout
|60
|Always, except when ONTAP DSM is detected
|HKLM\SYSTEM\CurrentControlSet\Control
...
|1|Always, except when ONTAP DSM is detected
...
|When MPIO is support-specified, except if ONTAP DSM is detected
|HKLM\SYSTEM\CurrentControlSet\Services\mpio
\Parameters\PathRecoveryInterval
|40
...
\msdsm\Parameters\PathVerifyEnabled
|0
|When MPIO support is specified and your server is Windows Server 2016, 2012 R2, 2012, 2008 R2, or 2008, except if ONTAP DSM is detected
|HKLM\SYSTEM\CurrentControlSet\Services
\msiscdsm\Parameters\PathVerifyEnabled
|0
|When MPIO support is specified and your server is Windows Server 2003, except if ONTAP DSM is detected
|HKLM\SYSTEM\CurrentControlSet\Services\vnetapp
\Parameters\PathVerifyEnabled
|0
|When MPIO support is specified, except if ONTAP DSM is detected
...
|When MPIO support is specified and your server is Windows Server 2016, 2012 R2, 2012, 2008 R2, or 2008, except if ONTAP DSM is detected
|HKLM\SYSTEM\CurrentControlSet\Services
\vnetapp\Parameters\RetryInterval |1 |When MPIO support is specified, except if ONTAP DSM is detected
1.2+|HKLM\SYSTEM\CurrentControlSet
\Services\disk\TimeOutValue |120 |When no MPIO support is selected, except if ONTAP DSM is detected |60 |When MPIO support is specified, except if ONTAP DSM is detected
|HKLM\SYSTEM\CurrentControlSet\Services\mpio
\Parameters\UseCustomPathRecoveryInterval
|1
