...
summary: Install the Linux Host Utilities on an Oracle Linux 9.0 host to manage FCP and iSCSI protocol operations with ONTAP LUNs.
---
= Configure Oracle Linux 9.0 for FCP and iSCSI with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
[.lead]
The Linux Host Utilities software provides management and diagnostic tools for Linux hosts that are connected to ONTAP storage. When you install the Linux Host Utilities on an Oracle Linux 9.0 host, you can use the Host Utilities to help you manage FCP and iSCSI protocol operations with ONTAP LUNs.
...
You can use multipathing with Oracle Linux 9.0 to manage ONTAP LUNs.

NOTE: You can use the link:hu_rhel_90.html#rhel-rhck[recommended settings for Red Hat Enterprise Linux (RHEL) 9.0] to configure Red Hat Compatible Kernel for Oracle Linux 9.0.
...
The Oracle Linux 9.0 with NetApp ONTAP storage release has the following known issues:
