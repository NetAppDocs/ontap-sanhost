...
For Red Hat Enterprise Linux (RHEL) 9.0 the `/etc/multipath.conf` file must exist, but you do not need to make specific changes to the file. RHEL 8.6 is compiled with all settings required to recognize and correctly manage ONTAP LUNs.
...
There are no know issues for RHEL 9.0.
