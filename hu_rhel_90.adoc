...
= Use Red Hat Enterprise Linux 9.0 with ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:nofooter:
...
[.lead]
You can use the ONTAP SAN host configuration settings to configure Red Hat Enterprise Linux 9.0 with ONTAP as the target. 
...
For Red Hat Enterprise Linux (RHEL) 9.0 the `/etc/multipath.conf` file must exist, but you do not need to make specific changes to the file. RHEL 9.0 is compiled with all settings required to recognize and correctly manage ONTAP LUNs.
...
The RHEL 9.0 OS is compiled to recognize ONTAP LUNs and automatically set all configuration parameters correctly for both ASA and non-ASA configuration.
