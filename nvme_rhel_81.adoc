...
[.lead]
NVMe/FC is supported on ONTAP 9.6 or later for Red Hat Enterprise Linux (RHEL) 8.1. A RHEL 8.1 host can run both NVMe and SCSI traffic through the same fibre channel initiator adapter ports. See the link:https://hwu.netapp.com/Home/Index[Hardware Universe^] for a list of supported FC adapters and controllers.

For the most current list of supported configurations, see the link:https://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix Tool^].
...
* NVMe multipath is disabled by default. Enable it manually. 

* By default, round-robin load balancing is not enabled. You must write a `udev` rule to enable this functionality. 
