...
The NVMe/FC host configuration for AIX with ONTAP release has the following known issues:
...
Before troubleshooting any NVMe/FC failures, verify that you are running a configuration that is compliant to the Interoperability Matrix Tool (IMT) specifications. If you are still facing issues, contact link:mysupport.netapp.com[NetApp support^] for further triage.
