...
Use the following procedure to validate the minimum supported Oracle Linux 7x software versions.

.Steps

. Install Oracle Linux 7x on the server. After the installation is complete, verify that you are running the specified Oracle Linux 7x kernel. 
+
[source,cli]
----
uname -r
...
. On the Oracle Linux 7x host, check the `hostnqn` string at `/etc/nvme/hostnqn`: 
+
[source,cli]
----
