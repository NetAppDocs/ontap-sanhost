...
|1479047	|RHEL 9.1 NVMe-oF hosts create duplicate Persistent Discovery Controllers	|On NVMe over Fabrics (NVMe-oF) hosts, you can create Persistent Discovery Controllers (PDCs) by passing the -p option to the "nvme discover" command. For a given initiator-target combination, the creation of one PDC is expected with each invocation of the "nvme discover" command. However, beginning with ONTAP 9.10.1, Red Hat Enterprise Linux (RHEL) 9.1 NVMe-oF hosts end up creating duplicate PDCs with each invocation of the "nvme discover" command with the -p option. This leads to wastage of resources on both the host and the target. 	|2087000
|1503468
|`nvme list-subsys` parameter returns repeated nvme controller list for a given subsystem.	|In RHEL 9.1, `nvme list-subsys` parameter returns nvme controllers with its respective ANA state for all namespaces, that belongs to a given subsystem. However, the ANA state is a per-namespace attribute hence it only makes sense to show it if the user lists the subsystem for a namespace.
Hence `nvme list-subsys` parameter returning the unique list of nvme controllers associated to a given subsystem would be more ideal.  	|2130106
|===
