...
For Oracle Linux (OL) 8.7 the `/etc/multipath.conf` file must exist, but you do not need to make specific changes to the file. OL 8.7 is compiled with all settings required to recognize and correctly manage ONTAP LUNs.
...
NOTE: Do not use an excessive number of paths to a single LUN. You should require no more than 4 paths. More than 8 paths might cause path issues during storage failures.
...
The Oracle Linux 8.7 OS is compiled to recognize ONTAP LUNs and automatically set all configuration parameters correctly for both ASA and non-ASA configuration.
...
NOTE: To configure Oracle Linux 8.7 RedHat Enterprise Kernel (RHCK), use the link:hu_rhel_86.html#recommended-settings[recommended settings] for Red Hat Enterprise Linux (RHEL) 8.7.
