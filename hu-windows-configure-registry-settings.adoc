...
[.lead]
The Windows Host Utilities require certain registry and parameter settings so that a Windows host correctly handles storage system behavior. 
...
* Beginning with Windows Host Utilities 7.1, the installer sets the Windows, FC, and NVMe/FC HBA parameters to the supported values for the Windows host. You need to manually set the iSCSI HBA parameters.
