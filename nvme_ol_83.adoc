...
. Verify that you are using the supported adapter. For the most current list of supported adapters, see the https://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix Tool^].
...
. Verify that you are using the recommended Broadcom lpfc firmware and inbox driver. For the most current list of supported adapter driver and firmware versions, see the https://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix Tool^].
+
----
...
. Verify that the initiator ports and able to see the target LIFs, and all are active and running.
...
include::_include/hu/reuse_nvme_enabling_broadcom_1mb_size.adoc[]
...
. Verify the NetApp plug-in displays correct values for each ONTAP namespace device.
+
----
...
include::_include/hu/reuse_nvme_verbose_logging.adoc[]

=== qla2xxx verbose logging

include::_include/hu/reuse_nvme__qla2xxx_verbose_logging.adoc[]
...
|===
|NetApp Bug ID |Title |Description |Oracle Bugzilla
|1517321 |Oracle Linux 8.x NVMe-oF Hosts create duplicate Persistent Discovery Controllers |
