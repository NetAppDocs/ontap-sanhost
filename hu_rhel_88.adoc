...
= Using Red Hat Enterprise Linux 8.8 with NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:nofooter:
...
For Red Hat Enterprise Linux (RHEL) 8.8, the `/etc/multipath.conf` file must exist, but you do not need to make specific changes to the file. RHEL 8.8 is compiled with all settings required to recognize and correctly manage ONTAP LUNs.
