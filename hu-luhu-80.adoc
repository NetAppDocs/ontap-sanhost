...
When you install the Linux Host Utilities, it doesn't change any settings or behavior on your host.
...
+
NOTE: The Linux Host Utilities has an installation dependency on the libHBAAPI.so library. You need to install one of the packages listed above, even if a host is intended for an iSCSI-only configuration. Alternatively, you can use OS-level tools for LUN and path management.  
...
.Ubuntu
--
.. Install Linux Host Utilities 8.0 on an Ubuntu host:
+
[source, cli]
----
...
.. Manually link the Ubuntu OS to the HBA library:
+
[source, cli]
----
