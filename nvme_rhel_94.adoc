...
* SAN booting using the NVMe-oF protocol is supported.
...
. The native inbox qla2xxx driver included in the RHEL 9.4 GA kernel has the latest fixes essential for ONTAP support. Verify that you are running the supported adapter driver and firmware versions:
+
----
...
NVMe/TCP does not have an auto-connect functionality. Therefore, we need to perform the NVMe/TCP connect or connect-all functionality manually to discover the NVMe/TCP subsystems and namespaces.
...
[NOTE]
Beginning with RHEL 9.4, the NVMe/TCP `ctrl_loss_tmo` timeout behaviour is set to infinity by default. Therefore, there is no need to configure a specific timeout period. With default behaviour NVMe/TCP will not timeout and will remain connected indefinitely, eliminating the need for setting a timeout for `nvme connect` or `nvme connect-all` (`argument -1`) in the event of path failure.
