...
summary: Describes how to use Red Hat Enterprise Linux 8.6 with ONTAP
---
= Use Red Hat Enterprise Linux 8.6 with ONTAP
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
[.lead]
You can use the ONTAP SAN host configuration settings to configure Red Hat Enterprise Linux 8.6 with ONTAP as the target. 
...
For Red Hat Enterprise Linux (RHEL) 8.6 the `/etc/multipath.conf` file must exist, but you do not need to make specific changes to the file. RHEL 8.6 is compiled with all settings required to recognize and correctly manage ONTAP LUNs.
...
The RHEL 8.6 OS is compiled to recognize ONTAP LUNs and automatically set all configuration parameters correctly for both ASA and non-ASA configuration.
...
There are no known issues for the RHEL 8.6 with ONTAP release.
