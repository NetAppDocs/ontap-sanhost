...
NVMe-oF Host Configuration for Oracle Linux 9.1 with ONTAP release has the following known issues:
...
|link:https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=17998[17998]
|1539101 |Oracle Linux 9.1 NVMe-oF hosts fails to create a persistent discovery controller | On Oracle Linux 9.1 NVMe over Fabrics (NVMe-oF) hosts, you can use the nvme discover -p command to create Persistent Discovery Controllers (PDCs). When this command is used, one PDC should be created per initiator-target combination. However, if you are running ONTAP 9.12.1 and Oracle Linux 9.1 with an NVMe-oF host, PDC creation fails when the `nvme discover -p` command is executed.  
|link:https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=18196[18196]
|===
