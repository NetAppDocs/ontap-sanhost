...
* By default, the ANATT value for the AIX host OS is 30 seconds. IBM provides an Interim Fix (ifix) which caps the ANATT value at 60 seconds; you need to install an ifix from the IBM website to ensure that all the ONTAP workflows are non-disruptive.. 
+

[NOTE]
For NVMe/FC AIX support, you need install an ifix on the GA versions of AIX OS. This is not required for the VIOS/PowerVM OS.
