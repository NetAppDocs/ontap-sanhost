...
summary: Describes how to use Oracle Linux 8.8 with ONTAP
---
...
For OL 8.8 the `/etc/multipath.conf` file must exist, but you do not need to make specific changes to the file. OL 8.8 is compiled with all the settings required to recognize and correctly manage ONTAP LUNs.
...
The Oracle Linux 8.8 OS is compiled to recognize ONTAP LUNs and automatically set all configuration parameters correctly for both ASA and non-ASA configurations. You can further optimize performance for your host configuration with the following recommended settings.
...
NOTE: To configure Oracle Linux 8.8 RedHat Enterprise Kernel (RHCK), use the link:hu_rhel_91.html#recommended-settings[recommended settings] for Red Hat Enterprise Linux (RHEL) 8.8.
