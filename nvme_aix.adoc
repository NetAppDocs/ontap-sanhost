...
* Set the ANATT (Asymmetric Namespace Access Transition Timeout) to 60 seconds or above to ensure that all the ONTAP workflows are non-disruptive. By default, the ANATT for an AIX host OS is 30s.
...
* IBM provides an interim iix (ifix) which caps the ANATT value at 60 seconds. For NVMe/FC AIX support, you must download and install the ifix from the IBM website on the GA versions of AIX OS. This is not required for the VIOS/PowerVM OS.
...
+
The following table demonstrates HBAs assigned to the AIX LPAR (AIX Logical Partition) or the physical/native stack in a non-virtualized mode:
+
