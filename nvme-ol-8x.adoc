...
.About this task
You can use the following support and features with the NVMe-oF host configuration for Oracle Linux 8x. You should also review the known limitations before starting the configuration process.
...
* Features available: 
** The in-kernel NVMe multipath feature is enabled for NVMe namespaces by default in Oracle Linux 8x. You don't need to configure explicit settings.
...
Validate the minimum supported software versions for Oracle Linux 8x.

.Steps

. Install Oracle Linux 8x GA on the server. After the installation is complete, verify that you are running the specified Oracle Linux 8x GA kernel:
+
----
uname -r
----
...
. Verify that `hostnqn` on the Oracle Linux 8x host matches `hostnqn` for the corresponding subsystem on the ONTAP array:
+
----
