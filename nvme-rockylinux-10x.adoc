...
summary: How to Configure NVMe-oF Host for Rocky Linux 10.x with ONTAP storage
---
...
Learn how to configure NVMe over Fabrics (NVMe-oF) hosts for Rocky Linux 10.x. For more support and feature information, see link:nvme-rockylinux-supported-features.html[Rocky Linux ONTAP support and features].
...
. Install Rocky Linux 10.x on the server. After the installation is complete, verify that you are running the required Rocky Linux 10.x kernel: 
+
[source,cli]
----
uname -r
...
Beginning with ONTAP 9.12.1, secure in-band authentication is supported over NVMe/TCP between a Rocky Linux 10.x host and an ONTAP controller.
...
cat /etc/nvme/hostnqn
----
. Generate the dhchap key for the Rocky Linux 10.x host.
+
