...
NVMe/FC is supported on ONTAP 9.6 or later for Oracle Linux 7.8. The Oracle Linux 7.8 host can run both NVMe and SCSI traffic through the same fibre channel (FC) initiator adapter ports. Note that the Broadcom initiator can serve both NVMe/FC and FCP traffic through the same FC adapter ports. See the link:https://hwu.netapp.com/Home/Index[Hardware Universe^] for a list of supported FC adapters and controllers. For the current list of supported configurations see the link:https://mysupport.netapp.com/matrix/[NetApp Interoperability Matrix^].
...
* There is no sanlun support for NVMe/FC and, as a consequence, no Linux Unified Host Utilities support for NVMe/FC on Oracle Linux 7.8. Use the ONTAP command output available as part of the NetApp plug-in included in the native nvme-cli.
...
== Enabling 1 MB I/O size for Broadcom NVMe/FC
