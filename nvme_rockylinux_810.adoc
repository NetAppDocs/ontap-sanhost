...
= NVMe-oF host configuration for Rocky Linux 8.10 with ONTAP
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
[.lead]
NVMe over Fabrics (NVMe-oF), including NVMe over Fibre Channel (NVMe/FC) and other transports, is supported with Rocky Linux 8.10 with Asymmetric Namespace Access (ANA). In NVMe-oF environments, ANA is the equivalent of ALUA multipathing in iSCSI and FC environments and is implemented with in-kernel NVMe multipath. 

The following support is available for NVMe-oF host configuration for Rocky Linux 8.10 with ONTAP:
...
* On Rocky Linux 8.10 hosts, NVMe/TCP is a technology preview feature due to open issues.
...
. Install Rocky Linux 8.10 on the host server. 
. After the installation is complete, verify that you are running the specified Rocky Linux 8.10 kernel: 
+
----
# uname -r
----
...
----------- --------------- ----------------------------------------------------------
vs_25_2742 rockylinux_101_QLe2772    nqn.2014-08.org.nvmexpress:uuid:546399fc-160f-11e5-89aa-98be942440ca
----
+
[NOTE]
...
The native inbox qla2xxx driver included in the Rocky Linux 8.10 GA kernel has the latest upstream fixes. These fixes are essential for ONTAP support.
