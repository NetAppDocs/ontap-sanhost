...
[.lead]
You can use the ONTAP SAN host configuration settings to configure Oracle Linux 8.1 with ONTAP as the target.
...
NOTE: To configure Oracle Linux 8.1 Red Hat Compatible Kernel (RHCK), use the link:hu_rhel_81.html#recommended-settings[recommended settings] for Red Hat Enterprise Linux (RHEL) 8.1.

== Known issues

There are no known issues for the Oracle Linux 8.1 with ONTAP release.

NOTE: For Oracle Linux RHCK known issues, see the link:hu_rhel_81.html#known-problems-and-limitations[known issues] for RHEL 8.1.
