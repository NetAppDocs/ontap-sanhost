...
|1479047	|RHEL 8.7 NVMe-oF hosts create duplicate Persistent Discovery Controllers	|On NVMe over Fabrics (NVMe-oF) hosts, you can create Persistent Discovery Controllers (PDCs) by providing the `-p` option to the `nvme discover` command. For a specified initiator-target combination, the creation of one PDC is expected with each invocation of the `nvme discover` command. However, beginning with ONTAP 9.10.1, Red Hat Enterprise Linux (RHEL) 8.7 NVMe-oF hosts end up creating duplicate PDCs with each invocation of the `nvme discover` command with the `-p` option. This leads to wastage of resources on both the host and target. 	|2087000
|===
