...
[.lead]
NVMe over Fabrics (NVMe-oF), including NVMe over Fibre Channel (NVMe/FC) and other transports, is supported with SUSE Linux Enterprise Server (SLES) 15 SP6 with Asymmetric Namespace Access (ANA). In NVMe-oF environments, ANA is the equivalent of ALUA multipathing in iSCSI and FCP environments and is implemented with in-kernel NVMe multipath.

The following support is available for the NVMe-oF host configuration for SLES 15 SP6 with ONTAP:
...
* There's no `sanlun` support for NVMe-oF. Therefore, the host utility support isn't available for NVMe-oF on an SLES 15 SP6 host. You can rely on the NetApp plug-in included in the native `nvme-cli` package for all NVMe-oF transports.
...
. The native inbox qla2xxx driver included in the SLES 15 SP6 kernel has the latest fixes essential for ONTAP support. Verify that you are running the supported adapter driver and firmware versions:
+
----
...
The `nvmefc-boot-connections.service` and `nvmf-autoconnect.service` NVMe/FC boot services are included in the `nvme-cli` package; however, only `nvmefc-boot-connections.service` is enabled by default to start during system boot.

Use the following procedure to manually enable `nvmf-autoconnect.service` to start during system boot. 
...
Process: 2114 ExecStart=/usr/sbin/nvme connect-all (code=exited, status=0/SUCCESS) Main 
PID: 2114 (code=exited, status=0/SUCCESS)
systemd[1]: Starting Connect NVMe-oF subsystems automatically during boot... nvme[2114]: 
traddr=nn-0x201700a098fd4ca6:pn-0x201800a098fd4ca6 is already connected systemd[1]: nvmfautoconnect.service: Deactivated successfully.
systemd[1]: Finished Connect NVMe-oF subsystems automatically during boot.
# systemctl status nvmefc-boot-connections.service
nvmefc-boot-connections.service - Auto-connect to subsystems on FC-NVME devices found during boot
Loaded: loaded (/usr/lib/systemd/system/nvmefc-bootconnections.service; enabled; vendor preset: 
enabled)
Active: inactive (dead) since Thu 2023-05-25 14:55:00 IST; 11min ago
Main PID: 1647 (code=exited, status=0/SUCCESS)
systemd[1]: Starting Auto-connect to subsystems on FC-NVME devices found during boot... systemd[1]: 
nvmefc-boot-connections.service: Succeeded. systemd[1]: Finished Auto-connect to subsystems on FC-NVME devices found during boot.
----
====

== Configure NVMe/TCP

NVMe/TCP does not have an auto-connect functionality. Therefore, you need to perform the NVMe/TCP connect or connect-all functionality manually to discover the NVMe/TCP subsystems and namespaces. 
...
---------------- ------------------------------------------------------------------------------------ -----------------------
nvme-subsys2 nqn.1992-08.com.netapp:sn.06303c519d8411eea468d039ea36a106:subsystem.nvme
nvme4, nvme6
Device SN MN FR TxPort Asdress Subsystem Namespaces
-------- -------------------- ---------------------------------------- -------- ---------------------------------------------
nvme4 81H75BVkrQflAAAAAAAB NetApp ONTAP Controller FFFFFFFF tcp traddr=192.168.111.66 
,trsvcid=4420,host_traddr=192.168.111.79 nvme-subsys2 nvme0n1
nvme6 81H75BVkrQflAAAAAAAB NetApp ONTAP Controller FFFFFFFF tcp 
traddr=192.168.111.67,trsvcid=4420,host_traddr=1192.168.111.79 nvme-subsys2 nvme0n1
Device Generic NSID Usage Format Controllers
------------ ------------ ---------- -------------------------------------------------------------
/dev/nvme0n1 /dev/ng0n1 0x1 1.07 GB / 1.07 GB 4 KiB + 0 B nvme4, nvme6
...
Beginning with ONTAP 9.11.1, you can create a PDC for your SLES 15 SP6 host. A PDC is required to automatically detect an NVMe subsystem add or remove scenario and changes to the discovery log page data. 
...
Beginning with ONTAP 9.12.1, secure in-band authentication is supported over NVMe/TCP and NVMe/FC between your SLES 15 SP6 host and your ONTAP controller.
...
. Generate the dhchap key for the SLES15 SP6 host:
+
----
...
There are no known issues for the SLES 15 SP6 with ONTAP release. 
