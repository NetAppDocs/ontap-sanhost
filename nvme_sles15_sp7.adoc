...
* Known limitations
** NetApp `sanlun` host utility support isn't available for NVMe-oF on a SUSE Linux Enterprise Server 15 SP7 host. Instead, you can rely on the NetApp plug-in included in the native `nvme-cli` package for all NVMe-oF transports.
...
+
----
nvme-cli-2.11+17.g6e09119-150700.1.1.x86_64
----
...
+
----
libnvme1-1.11-150700.2.2.x86_64
----
...
Beginning with ONTAP 9.12.1, secure in-band authentication is supported over NVMe/TCP and NVMe/FC between a SUSE Linux Enterprise Server 15 SP7 host and an ONTAP controller.
...
.. `rpm -qa | grep ktls`
+
You should see the following `ktls-utils`:
+
----
