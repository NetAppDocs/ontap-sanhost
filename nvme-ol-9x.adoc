...
summary: NVMe-oF Host Configuration for Oracle Linux 9x with ONTAP
---
= Configure Oracle Linux 9x with NVMe-oF for ONTAP storage 
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
The following support and features are available with the NVMe-oF host configuration for Oracle Linux 9x with ONTAP storage.

* Support and features available:
...
** Oracle Linux 9x enables in-kernel NVMe multipath for NVMe namespaces by default, removing the need for explicit settings.
** Beginning with ONTAP 9.12.1 and Oracle Linux 9.4, support for secure in-band authentication is introduced for NVMe/TCP. You can use secure in-band authentication for NVMe/TCP.
...
NVMe-oF with OL 9x has the following known limitations:
...
Use the following procedure to validate the minimum supported Oracle Linux 9x software versions.

.Steps

. Install Oracle Linux 9x on the server. After the installation is complete, verify that you are running the specified Oracle Linux 9x kernel. 
+
[source,cli]
----
uname -r
...
.	On the Oracle Linux 9x host, check the `hostnqn` string at `/etc/nvme/hostnqn`: 
+
[source,cli]
----
...
Beginning with ONTAP 9.12.1, secure in-band authentication over NVMe/TCP is supported between an ONTAP controller and Oracle Linux hosts starting with version 9.4.
