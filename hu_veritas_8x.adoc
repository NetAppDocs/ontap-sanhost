...
Use Veritas Dynamic Multipathing (VxDMP) with Veritas Infoscale 8 to manage ONTAP LUNs.

To ensure that VxDMP is configured correctly for your host, you need to verify the VxDMP configuration and check the Array Support Library (ASL) and the Array Policy Module (APM) configuration. The ASL and APM packages for NetApp storage systems are installed during the Veritas software installation.
...
. Set the host "rport" values for RHEL 8.x and 9.x:
+
Configure `udev rport` values to support the Veritas Infoscale environment in storage failover scenarios by creating the file `/etc/udev/rules.d/40-rport.rules` with the following file content:
+
----
