...
This document describes how to configure NVMe over Fabrics (NVMe-oF) hosts for RHEL 9.0. For additional support and feature information, refer to link:hu-nvme-index.html[NVME-oF Overview^]. The following are known limitations for RHEL 9.0:
...
You can use the following procedure to verify the minimum supported RHEL 9.0 software versions and NVMe configuration.
...
[NOTE]
NVMe/FC is supported on RHEL 9.0 and later. NVMe/TCP is supported on RHEL 8.6 and later. NVMe/TCP doesn't support auto-connect operations and requires manual connect or connect-all commands.
...
. Verify that the initiator port can retrieve the discovery log page data across the supported NVMe/TCP LIFs:
+
[source,cli]
----
...
You can enable I/O requests of size 1MB for NVMe/FC configured with a Broadcom adapter, see  link:nvme-enable-broadcom-1mb-size.html[Enable 1MB I/O for NVMe/FC^].

== Step 5: Verify NVMe boot services

With RHEL 9.0, the `nvmefc-boot-connections.service` and `nvmf-autoconnect.service` boot services included in the NVMe/FC `nvme-cli` package are automatically enabled when the system boots. 
