...
[.lead]
NVMe over Fabrics (NVMe-oF), including NVMe over Fibre Channel (NVMe/FC) and other transports, is supported with SUSE Linux Enterprise Server (SLES) 15 SP4 with Asymmetric Namespace Access (ANA). 

You can configure the NVME-oF hosts for SUSE Linux Enterprise Server (SLES) 15 SP4 with ONTAP as the target.

In NVMe-oF environments, Asymmetric Namespace Access (ANA) is the equivalent of ALUA multipathing in iSCSI and FCP environments and is implemented with in-kernel NVMe multipath.
...
Between the two NVMe/FC boot services included in the `nvme-cli` package, you can enable _only_ `nvmefc-boot-connections.service` to start during system boot, but not `nvmf-autoconnect.service`. Therefore, you need to manually enable `nvmf-autoconnect.service` to start during system boot.

.Steps

. Execute the following command to enable `nvmf-autoconnect.service`:
+
----
...
. Reboot the host. 
. Verify that `nvmf-autoconnect.service` and `nvmefc-boot-connections.service` are running after bootup. 
+
*Example*
+
----
...
+
[NOTE]
NetApp recommends setting the `ctrl-loss-tmo` option to -1 so that the NVMe/TCP initiator attempts to indefinitely try to reconnect in the event of a path loss.
...
----
nvme list
nvme list -v # new output to be shared
----
+
*Example output:*
+
...
Beginning with ONTAP 9.11.1, you can create a persistent discovery controller for your SLES 15 SP4 host.
...
. A persistent discovery controller (PDC) is reqired to automatically detect NVMe subsystem add or remove scenarios and changes to the discovery log page data. You can create a PDC to the discovery subsystem: 
+
----
...
Alternatively config.json file can be used in conjunction with `nvme connect-all` command when multiple NVMe subsystems are available on the ONTAP controller configuration. 

You can generate the json file using -o option. refer to man pages for more syntax options.
--
