...
Learn how to configure NVMe over Fabrics (NVMe-oF) hosts for Oracle Linux 9x. For more support and feature information, see link:nvme-ol-supported-features.html[Oracle Linux ONTAP support and features].
NVMe-oF with Oracle Linux 9x has the following known limitation:
...
Beginning with Oracle Linux 9.6, the host sets the default iopolicy for NVMe-oF to `round-robin`. If your host is Oracle Linux 9.6 or later, you can change the iopolicy to `queue-depth` by modifying the udev rules file.
...
Beginning with Oracle Linux 9.4, secure in-band authentication is supported over NVMe/TCP between an Oracle Linux host and an ONTAP controller.
