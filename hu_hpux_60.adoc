...
[NOTE] 
To make it immediately clear which environment is being used, this document sometimes specifies “DMP” for the Veritas DMP environment and “MPIO” for the HP-UX native environment. In some cases, the commands you use might vary depending on which drivers you are using. In those cases, both the environment and driver types are specified.
...
Refer to the link:hu_solaris_62_cmd.html[sample command reference] for end-to-end validation of your storage configuration.
