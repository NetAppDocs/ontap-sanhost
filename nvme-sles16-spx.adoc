...
[NOTE]
====
Beginning with SUSE Linux Enterprise Server 16, the setting for the NVMe/TCP `ctrl_loss_tmo timeout` is automatically set to "off". As a result:
...
[source,cli]
----
nvme subsystem controller show -vserver sles16_tls -subsystem sles16 -instance
----
+
.Show example output
[%collapsible]
