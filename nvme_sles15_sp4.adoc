...
+
[NOTE]
NetApp recommends setting the `ctrl-loss-tmo` option with an indefinite value (such as -1) so that the NVMe subsystem keeps trying to reconnect in the event of a path loss. 
