...
[.lead]
You can use the ONTAP SAN host configuration settings to configure CentOS 8.3 with ONTAP as the target.
...
For CentOS 8.3 the /etc/multipath.conf file must exist, but you do not need to make specific changes to the file. CentOS 8.3 is compiled with all settings required to recognize and correctly manage ONTAP LUNs.
...
The CentOS 8.3 OS is compiled to recognize ONTAP LUNs and automatically set all configuration parameters correctly for both ASA and non-ASA configuration.
