...
summary: Install Linux Host Utilities on a Rocky Linux 9.x host to manage FCP and iSCSI protocol operations with ONTAP LUNs.
---
= Configure Rocky Linux 9.x for FCP and iSCSI with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
[.lead] 
The Linux Host Utilities software provides management and diagnostic tools for Linux hosts that are connected to ONTAP storage. When you install Linux Host Utilities on a Rocky Linux 9.x host, you can use the Host Utilities to help you manage FCP and iSCSI protocol operations with ONTAP LUNs.
...
You can use multipathing with Rocky Linux 9.x to manage ONTAP LUNs.
...
Rocky Linux 9.x with ONTAP storage has the following known issues.
