...
summary: Describes how to use Citrix XenServer 8.2 with ONTAP
---

= Configure Citrix Xenserver 8.2  for FCP and iSCSI with ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
The Linux Host Utilities software provides management and diagnostic tools for Linux hosts that are connected to ONTAP storage. When you install the Linux Host Utilities on a Citrix Xenserver 8.2  host, you can use the Host Utilities to help you manage FCP and iSCSI protocol operations with ONTAP LUNs.
...
You can use multipathing with Citrix Xenserver 8.2 to manage ONTAP LUNs.

For Citrix Xenserver 8.2  the `/etc/multipath.conf` file must exist, but you do not need to make specific changes to the file. Citrix Xenserver  8.2 is compiled with all settings required to recognize and correctly manage ONTAP LUNs.
...
There are no known issues for the FC host configuration for Citrix Xenserver 8.2 with ONTAP release.
