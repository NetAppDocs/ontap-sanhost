...
. By default, the Proxmox OS-native multipath configuration sets `find_multipaths` to "strict" with an empty zero byte /etc/multipath.conf configuration file. This can prevent ONTAP LUNs from being discovered as multipath devices, which means they might not appear under multipath control automatically.
+
To ensure that ONTAP LUNs are correctly recognized and managed by multipathd, you must override the default `find_multipaths` parameter: 
