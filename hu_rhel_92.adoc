...
= Using Red Hat Enterprise Linux 9.2 with NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:nofooter:
...
The NetApp Linux Unified Host Utilities software package is available on the link:https://mysupport.netapp.com/site/downloads[NetApp Support Site^] in a 64-bit .rpm file. 

Installing the Linux Unified Host Utilities is strongly recommended by NetApp, but not mandatory. The utilities do not change any settings on your Linux host. The utilities improve management and assist NetApp customer support in gathering information about your configuration.
...
. Download the 64-bit Linux Unified Host Utilities software package from the link:https://mysupport.netapp.com/site/downloads[NetApp Support Site] to your host.
...
The RHEL 9.2 OS is compiled to recognize ONTAP LUNs and automatically set all configuration parameters correctly for both ASA and non-ASA configurations.
...
|1508554   	|NetApp Linux Host Utilities CLI requires additional library package dependencies to support Emulex HBA adapter discovery	|In RHEL 9.2, the NetApp Linux SAN host utilities CLI `sanlun fcp show adapter -v` fails because the library package dependencies to support HBA discovery cannot be found. |N/A
|1537359 |LVM hung tasks with SAN Boot host after issuing NetApp giveback With RHEL 9.2 SAN Boot host, |Kernel disruption occurs when LVM hung tasks are seen with sanbooted Emulex HBA after storage failover giveback operation on a Red Hat Enterprise Linux 9.2. The kernel disruption causes the operating system to reboot and generates the vmcore file under the /var/crash/directory if kdump is configured.
|2173947
|===
