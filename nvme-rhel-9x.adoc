...
Beginning with RHEL 9.6, the host sets the default iopolicy for NVMe-oF to `round-robin`. If your host is Oracle Linux 9.6 or later, you can change the iopolicy to `queue-depth` by modifying the udev rules file.
...
Beginning with RHEL 9.3, secure in-band authentication is supported over NVMe/TCP between the RHEL host and ONTAP controller.
