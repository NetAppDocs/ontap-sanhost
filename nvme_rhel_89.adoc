...
The following support is available for the NVMe-oF host configuration for RHEL 8.9 with ONTAP:
...
+
----
# grubby --args=nvme_core.multipath=Y --update-kernel /boot/ vmlinuz-4.18.0-509.el8.x86_64
----
...
. The native inbox qla2xxx driver included in the RHEL 8.9 GA kernel has the latest upstream fixes essential for ONTAP support. Verify that you are running the supported adapter driver and firmware versions:
+
----
