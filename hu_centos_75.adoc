...
summary: Install Linux Host Utilities on an CentOS 7.5 host to manage FCP and iSCSI protocol operations with ONTAP LUNs.
---
= Configure CentOS 7.5 for FCP and iSCSI for ONTAP storage
:hardbreaks:
:toclevels: 1
:nofooter:
:icons: font
...
[.lead]
The Linux Host Utilities software provides management and diagnostic tools for Linux hosts that are connected to ONTAP storage. When you install the Linux Host Utilities on a CentOS 7.5 host, you can use the Host Utilities to help you manage FCP and iSCSI protocol operations with ONTAP LUNs.
...
You can use multipathing with CentOS 7.5 to manage ONTAP LUNs.
...
The CentOS 7.5 host with ONTAP storage has the following known issues:
